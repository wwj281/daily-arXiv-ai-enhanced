...
TL;DR: 本文提出了GAND，一个性别模糊的自然数据基准资源，用于评估机器翻译系统在处理缺乏明确性别线索的文本时的性别偏见。通过将英语源句翻译成两种语法性别语言并辅以对比翻译，进行特征归因分析，揭示了上下文中影响目标翻译中模糊指代实体性别翻译的关键源词。


<details>
  <summary>Details</summary>
Motivation: 机器翻译系统持续产生性别偏见的翻译，在自我表达至关重要的时代，基于默认行为和刻板印象的错误翻译可能对用户造成伤害。需要能够自然反映性别模糊场景的基准资源来更好地理解这些系统在缺乏明确性别线索时如何翻译性别。

Method: 构建GAND数据集（性别模糊自然数据基准资源），包含英语源句；将子集翻译为两种语法性别语言并扩展人工对比翻译；进行特征归因分析，识别上下文中影响目标翻译中模糊指代实体性别翻译的源词。

Result: 成功构建了GAND基准资源；特征归因分析揭示了影响性别翻译的关键上下文源词。

Conclusion: GAND资源可用于分析上下文线索对翻译中性别的影响，有助于理解和缓解机器翻译系统的性别偏见。
...
TL;DR: 本文提出了MioFFAn，一个基于MioGatto架构的开源文档中心框架，用于数学表达式到可执行符号代码的自动翻译（公式形式化）的快速标注，通过可定制分类法、部分LLM自动化和人在回路方法解决技术科学领域高质量数据集稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 科学文献中数学表达式自动翻译为可执行符号代码（公式形式化）受到技术科学领域高质量标注数据集严重稀缺的阻碍。

Method: 基于MioGatto架构扩展功能，引入方程选择和辅助符号代码规范等专用功能；支持用户配置自定义分类法和符号属性；通过LLM定义模块化自动子任务实现部分自动化，并采用人在回路方法进行迭代优化。

Result: 初步评估证明了该人在回路方法的有效性。

Conclusion: MioFFAn是一个可定制的开源框架，能够适应不同技术科学领域，并通过部分自动化和人在回路设计支持研究者迭代改进自动化能力。
...
TL;DR: 本研究分析了不同类型审稿指南（官方会议指南和基于LLM生成的模拟人类审稿指南）对自动化同行评审的影响，发现官方会议指南效果最佳，而强制使用严格评分标准会降低性能。


<details>
  <summary>Details</summary>
Motivation: 同行评审工作量日益增长，自动化评审需求迫切，但不同类型审稿指南对自动化评审效果的影响尚不清楚。

Method: 对比官方会议指南与基于高质量人工评审使用LLM生成的模拟审稿指南，在自动化评审中的表现差异，并测试严格评分标准的效果。

Result: 官方会议指南产生的评审结果与人类判断一致性最高；模拟审稿指南效果不如官方指南；强制严格评分标准持续降低性能。

Conclusion: 经过会议实践打磨的评估准则可作为自动化评审的有效指导，而主观、整体的评分方式优于严格的评分标准。
...
TL;DR: AutoThinkSQL通过集成自动思考机制到SFT和DPO中，让模型对简单查询跳过推理、对复杂查询使用深度CoT，在Spider和BIRD基准上超越最佳基线，同时减少24.6%/18.3%的输出token和17.1%/11.5%的延迟。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL方法对简单查询也强制使用Chain-of-Thought推理，造成不必要的推理开销和延迟，而实际中大量查询是简单查询，无需多步推导。

Method: 在监督微调(SFT)和直接偏好优化(DPO)阶段引入自动思考机制，让模型根据查询难度动态决策：简单查询跳过推理直接生成SQL，复杂查询调用深度CoT进行逐步推理。

Result: 在Qwen3-Coder-30B-A3B上，相比最佳基线，在Spider和BIRD基准上获得一致提升，同时平均输出token减少24.6%和18.3%，平均延迟降低17.1%和11.5%。

Conclusion: 模型学会了将推理决策与查询难度对齐，在保持高性能的同时显著降低了推理成本，验证了动态推理策略在Text-to-SQL任务中的有效性。
...
TL;DR: 本文提出LENS评估协议，用于测试大语言模型在机器遗忘后是否会复述特定叙事框架（如俄罗斯对乌克兰战争、美国对台湾政策），并引入SCE分数作为检查点选择指标。实验发现遗忘可减少叙事复述，但也存在实体恢复等副作用。


<details>
  <summary>Details</summary>
Motivation: 大语言模型能够复述与虚假信息一致的叙事框架，现有机器遗忘算法能否抑制这一行为尚不明确，需要系统的评估方法来检验。

Method: 提出LENS评估协议，包含直接、归因、对比和抽象抵抗四个层次；引入SCE分数平衡叙事抑制与输出质量；在四种12B参数多语言指令模型上评估两种源叙事框架。

Result: 选中的检查点能减少叙事复述，抑制效果可迁移至直接遗忘提示之外；但存在实体恢复副作用，即抽象A/B/C提示可能导致模型恢复目标框架中的真实世界行动者。
...
TL;DR: 本文提出了PatiGonit22K，一个包含22,441道数学应用题的大规模孟加拉语数据集，通过扩展原始PatiGonit数据集，涵盖简单与多步运算，为低资源语言的数学推理研究提供了更全面的基准资源。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语等低资源语言缺乏大规模标注的数学应用题数据集，限制了自然语言理解和数学推理研究的发展。

Method: 通过扩展原始PatiGonit数据集，收集了22,441道数学应用题，包含简单和多步运算方程，对每个问题进行了翻译、标注、文化适应和验证以确保语言一致性和数学正确性。

Result: 构建了PatiGonit22K数据集，包含22,441个问题，覆盖不同难度级别，为孟加拉语数学推理提供了更加全面的资源。

Conclusion: PatiGonit22K通过提升数据规模和复杂度，为低资源语言的数学推理研究和教育NLP应用提供了重要的基准资源。
...
TL;DR: CHiPS是一种轻量级、基于字符级的罗马尼亚语作者归属方法，利用字符直方图（CH-SVM）和位置信号（FFT12-LR）两种互补特征，在严格泄露控制下达到0.9310准确率（CHiPS-F），不依赖分词、句法分析、预训练模型或Transformer微调。


<details>
  <summary>Details</summary>
Motivation: 探索在严格信息泄露控制下，受限但透明的字符级证据（如字符直方图和位置信号）在作者归属任务中能达到何种性能，而非追求最佳分类准确率。

Method: 提出CHiPS方法，包括两个互补分类器：CH-SVM（基于单字符边缘分布的特征直方图SVM分类器）和FFT12-LR（将选定字符和标点类别表示为脉冲序列，提取傅里叶/Welch频谱描述符的逻辑回归分类器）；并融合为CHiPS-F，可选top-5列表式重排序器。避免分词、句法分析、预训练模型、Transformer微调及n≥2的字符n-gram特征。

Result: 在400个文件（10位作者）的ROST数据集上，CHiPS-F准确率0.9310，macro-F1 0.9341；在1,248个文件（19位作者）的ROStories-cleaned数据集上，CHiPS-R准确率0.8919，macro-F1 0.8708。对比的无限制字符2-5-gram TF-IDF SVM达到1.0000准确率。

Conclusion: 受限但透明的字符级特征在严格泄露控制下能达到有竞争力的作者归属性能，但并非最优分类准确率；贡献在于评估受限特征在严格泄漏控制下的潜力。
...
TL;DR: 本文提出使用 nuisance attribute projection (NAP) 进行语言归一化，在跨语言说话人验证中将开发集 EER 从 2.97% 降至 2.18%，在 TidyVoice2026 挑战赛中取得 8.40 的评估分数。


<details>
  <summary>Details</summary>
Motivation: 跨语言不匹配是当前说话人验证系统性能下降的主要原因，TidyVoice2026 挑战赛专门针对这一场景，要求在不提供语言标签的情况下进行文本无关的说话人验证。

Method: 在 SimAM-ResNet34 基线模型的基础上，从跨语言相同说话人的嵌入差异中估计一个紧凑的语言子空间，然后将嵌入投影到该子空间的正交补上，最后结合自适应对称分数归一化（AS-Norm）进行余弦打分。

Result: 开发集 EER 从余弦打分的 2.97% 和 AS-Norm 的 2.70% 降至 2.18%，Codabench 评估分数为 8.40。

Conclusion: 简单的后端语言归一化方法（NAP）能够与更复杂的系统相媲美，有效缓解跨语言说话人验证中的语言不匹配问题。
...
TL;DR: 前沿语言模型已展现出利用语义无关的填充词（filler tokens）进行“隐形推理”的能力，即在输出token中不留下可解释痕迹的计算过程。该研究在13个前沿模型上验证了这一现象，发现填充词可提升最多13个百分点的任务准确率，并能使Claude Opus 4.5在隐藏算术约束下保持主任务性能，表明隐形推理可以服务于CoT监控完全不可见的目标。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型是否所有推理都显式表达在输出token中，验证是否存在“隐形推理”（invisible reasoning）这一AI安全关键问题。

Method: 设计了三项合成推理任务，评估13个前沿语言模型在引入语义无关填充词（filler tokens）前后的性能变化；进一步设置隐藏模运算约束任务，测试隐形推理能否服务于CoT监控不可见的目标；并通过RL和SFT实验分析填充词偏好的来源。

Result: 多个模型受益于填充词，准确率提升高达13个百分点；Claude Opus 4.5能利用填充词满足隐藏的模运算约束而主任务精度不降；RL使Qwen3-235B对填充词内容产生强烈偏好，但RL和SFT均无法使填充词收益在测试时持久保持。

Conclusion: 前沿模型已经能够在输出token中执行无解释痕迹的重要计算，这对依赖输出token监控（如CoT）的AI安全方法构成实质性挑战。
...
### [81] [Fast Iterative Dual-Output-Aware LUT Mapping for Fracturable FPGA Architectures](https://arxiv.org/abs/2607.22756)
*Sheng Lu*

Main category: cs.AR

TL;DR: 本文提出一种面向可重写查找表（fracturable LUT）的双输出感知LUT映射框架，通过迭代式的单输出裁剪选择与有界双输出匹配，在ABC映射器中实现了面积（最高34.96%）、深度（约5%）和速度（15.8倍加速）的综合提升。


<details>
  <summary>Details</summary>
Motivation: 传统技术映射流程通常先进行单输出LUT映射，再进行双输出打包，导致潜在的配对机会无法影响裁剪选择，从而限制了面积与延迟优化空间。

Method: 提出迭代式双输出感知LUT映射框架：(1) 交替执行单输出裁剪选择和基于稀疏支持索引的有界双输出匹配；(2) 通过参数化结构约束、依赖约束和输出特定时序约束验证候选对；(3) 采用启发式评分匹配选择最佳配对；(4) 将配对信息通过兼容性感知的裁剪代价调整反馈到后续映射轮次；(5) 物理输入合并仅用于结构合法性检查，每个输出保留独立逻辑时序支持。

Result: 在EPFL组合基准测试集上，相比原始ABC，平均LUT面积减少34.96%和23.39%；相比已有最优方法，加速15.8倍，深度减少约5%，面积减少1%。

Conclusion: 将双输出感知提前到映射阶段而非映射后打包，能显著提升fracturable LUT的利用率，且所提方法在面积、延迟和运行效率上均优于现有方案。
...
### [82] [Benchmarking LLMs for Verilog Design Flows](https://arxiv.org/abs/2607.22759)
*Angshuman Chakravertty,Rahul Koshti,Buddhi Prakash Sharma,Vinay Chamola*

Main category: cs.AR

TL;DR: 本文提出了一个可复现的基准测试平台，用于评估开源大语言模型在Verilog RTL代码生成方面的能力。该平台包含50个精心设计的任务，并通过约束提示、后处理、语义感知迭代优化（波形分析、形式等价验证、AST修复）等流程进行端到端工具链验证。在12个基准测试、1610次运行中测试了3种不同规模的模型，语法有效性从0%提升至平均70.43%，仿真通过率达到51.8%。值得注意的是，1.1B参数的TinyLlama在语法有效性上达到80.0%，功能性正确性与8B模型相当。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在代码生成方面表现出潜力，但其生成正确、可综合的硬件描述语言（HDL）代码的能力尚未得到充分基准测试。现有评估主要依赖pass@k指标，缺乏完整的端到端工具链验证。

Method: 提出了一个可复现的基准测试平台，包括：1) 50个涵盖组合、时序、有限状态机和混合设计的Verilog RTL任务；2) 包含约束提示、后处理和语义感知迭代优化（波形分析、形式等价验证、AST修复）的流水线；3) 使用Verilator编译和Icarus Verilog仿真进行验证。

Result: 在12个基准测试、1610次运行中评估了Llama-3-8B、StarCoder2-7B和TinyLlama-1.1B三个模型。语法有效性从0%提升至平均70.43%，仿真通过率达到51.8%。TinyLlama（1.1B）达到最高语法有效性80.0%，功能性正确性与8B模型相当。

Conclusion: 该平台和数据集是开源的，能够实现对生成式AI在硬件设计工作流中的可复现评估。实验表明小型模型在HDL生成任务中具有竞争力，且端到端工具链验证对评估代码可合成性和功能正确性至关重要。
...
### [83] [DRC-Aid: Design-Rule Correction via Agentic Framework utilizing Inference-Time Large Language Models](https://arxiv.org/abs/2607.22761)
*Anushka Mukherjee,Kang He,Kaushik Roy*

Main category: cs.AR

TL;DR: DRC-Aid是一个闭环智能体框架，通过将设计规则违例修复形式化为验证在环搜索，利用规则引擎、大语言模型和物理验证工具反馈，在FreePDK45布局上实现了约92.5%的DRC清洁、LVS等效修复，总违例减少约98%。


<details>
  <summary>Details</summary>
Motivation: 布局中的设计规则违例(DRV)修复需要反复进行几何编辑和验证的迭代循环，该过程繁琐且耗时，现有方法缺乏自动化、高效的闭环修复机制。

Method: 提出DRC-Aid闭环智能体框架：(1) 确定性规则引擎将物理验证工具报告的违例转换为有界几何编辑菜单；(2) 预训练LLM评估局部几何上下文，从菜单中选择编辑操作；(3) 采用带预算的深度优先搜索和回溯策略；(4) 物理验证工具(Calibre nmDRC/nmLVS)提供即时反馈，确保几何合规并防止电气拓扑退化；(5) 全局记忆库防止循环重复探索。

Result: 在FreePDK45布局上，DRC-Aid实现约92.5%的DRC清洁、LVS等效修复，总违例减少约98%。LLM驱动的选择策略优于随机(54.4%)和确定性启发式(83.3%)策略，在含6个及以上违例的案例中差距更明显。

Conclusion: DRC-Aid通过将LLM的几何上下文理解能力与物理验证工具的精准反馈相结合，有效实现了布局DRV的自动化修复，验证了闭环智能体框架在电子设计自动化中的应用潜力。
...
### [84] [FusionML: Prefill, Not Decode - Mechanism and Boundaries of CPU+GPU Co-Execution on Unified-Memory Apple Silicon](https://arxiv.org/abs/2607.22785)
*Om Mohite*

Main category: cs.AR

TL;DR: 本文发现MLX惰性图调度器会序列化跨流任务，导致CPU+GPU混合加速失效；通过引入急切物化边界，实现了基于逐层、感知争用的行分割预填充加速，在五款芯片上获得1.15-1.38倍加速，且首次Token延迟提升1.18-1.25倍，同时保持输出一致和解码吞吐不变。


<details>
  <summary>Details</summary>
Motivation: Apple Silicon SoC共享CPU、GPU和神经引擎的统一内存系统，此前尝试将Transformer推理中的单个算子拆分到不同单元加速均失败或产生精度混淆的成果。本文旨在揭示失败根因并提出有效的异构加速方案。

Method: 识别MLX惰性图调度器在CPU流操作消费未物化的GPU结果时会序列化跨流工作的根本问题；提出急切物化边界恢复并发性；实现逐层、感知争用的CPU+GPU行分割方案用于Transformer预填充。

Result: 在五款芯片（覆盖三代Apple Silicon）上，Llama风格解码器块的预填充加速1.15-1.38倍，满32层深度不变；真实Qwen2.5-7B检查点上首次Token延迟提升1.18-1.25倍；输出token一致且解码吞吐不变；解码阶段无收益；精度匹配训练损失0.86-0.97倍；ANE调度开销排除在层粒度使用。

Conclusion: 通过修复MLX惰性图调度导致的序列化问题，CPU+GPU异构加速在满足急切物化边界条件下可有效加速Transformer预填充，但解码阶段、训练和ANE均无法从中受益，且内存压力下无退化运行时门控会自败。
...
### [85] [The Sparsity Tax: Weight Sparsity Trade-offs in Event-Driven SIMD and SIMT Neuromorphic Cores](https://arxiv.org/abs/2607.22790)
*Mattias Westerink,Sameed Sohail,Berend-Jan van der Zwaag,Sabih Gerez,Amirreza Yousefzadeh*

Main category: cs.AR

TL;DR: 本文量化了事件驱动神经形态推理中由于权重稀疏性带来的"稀疏性税"（控制、元数据和存储开销），通过比较三种加速器设计（基线SIMD、稀疏SIMD和SIMT），发现在高稀疏度下SIMT设计在能效和速度上最优，但存在次线性增益问题。


<details>
  <summary>Details</summary>
Motivation: 事件驱动神经形态推理利用激活稀疏性提升效率，但权重稀疏性引入不规则聚集式更新，破坏了锁步SIMD执行的效率，产生了控制、元数据和存储活动方面的额外开销（即"稀疏性税"），需要量化并探索更优的加速器架构。

Method: 在同一神经形态核心中集成三种加速器设计：(i)基线锁步SIMD，(ii)基于位图门控的稀疏SIMD（选择性地禁用通道但不压缩权重），(iii)SIMT风格设计（每PE独立地址生成+游程编码稀疏权重）。使用GF22FDX+的RTL-to-gates流程和活动驱动能量估计，评估不同后训练剪枝水平下的事件驱动神经网络推理。

Result: 总面积变化不显著（SRAM占主导）；SIMD和稀疏SIMD吞吐量近乎恒定；稀疏SIMD因位图和密集存储开销能效提升有限；SIMT在高稀疏度下能效缩放最佳且速度提升显著，但因元数据读取、负载不均和稀疏无关阶段存在次线性增益。

Conclusion: 稀疏性处理方式对性能和能效影响重大，SIMT架构在高稀疏度场景下具有优势但需解决次线性增益问题；所有硬件代码已开源供研究使用。
...
### [86] [Reducing Instruction-Fetch Energy in RISC-V for Embedded AI Processing via Dynamic and Static Loop Caching](https://arxiv.org/abs/2607.22792)
*Wiebren Wijnstra,Sameed Sohail,Berend-Jan van der Zwaag,Sabih Gerez,Amirreza Yousefzadeh*

Main category: cs.AR

TL;DR: 本文提出了两种集成在RISC-V处理器数据路径中的循环缓存架构（动态循环缓存和静态循环缓存），用于减少指令获取能耗。在GlobalFoundries 22nm FDX+工艺上对LeNet-5 CNN推理工作负载的评估显示，动态缓存减少48.3%的指令获取和21.5%的总能耗，静态缓存减少83.3%的指令获取和35.5%的总能耗，面积开销均低于SoC总面积的0.2%。全部实现已开源。


<details>
  <summary>Details</summary>
Motivation: 嵌入式RISC-V处理器在边缘AI推理场景中，从SRAM存储器中获取指令是核心能耗的主要来源（在基线测量中占总能耗的40%以上），因此需要高效的指令缓存机制来降低能耗。

Method: 在RISC-V处理器数据路径中集成了两种循环缓存架构：动态循环缓存（自动检测并缓存运行时短后向分支循环）和静态循环缓存（软件管理的热代码指令缓冲区，允许在引导序列期间预加载任意指令块）。两者均在开源NEORV32 RISC-V处理器上实现。

Result: 动态缓存减少48.3%的指令获取和21.5%的总能耗；静态缓存减少83.3%的指令获取和35.5%的总能耗。两种设计的面积开销均低于SoC总面积的0.2%。

Conclusion: 两种循环缓存架构能有效降低边缘AI推理中RISC-V处理器的指令获取能耗，且面积开销极小。静态缓存在能耗节省上优于动态缓存，但需要软件管理支持。
...
### [87] [Multi-primitive in-memory computing for Monte Carlo tree search](https://arxiv.org/abs/2607.22869)
...
TL;DR: 本文提出"阶段到原语分解"方法，将MCTS算法的四个阶段（选择、扩展、回滚、反向传播）分别映射到内容寻址存储器、组合逻辑、RRAM交叉阵列和SRAM等存内计算原语上，在22nm工艺下实现仅~60mW功耗的IMC-MCTS系统，相比CPU和GPU分别获得96倍和65-2059倍的能效提升，并达到与开源围棋引擎相当的竞技水平。


<details>
  <summary>Details</summary>
Motivation: 传统MCTS在CPU/GPU上运行功耗高达55-300W，难以部署在边缘设备上。虽然存内计算(IMC)在常规工作负载上能效优异，但被认为与不规则的多阶段算法不兼容。

Method: 提出"阶段到原语分解"(phase-to-primitive decomposition)方法，将MCTS的选择、扩展、回滚和反向传播四个阶段分别映射到CAM、组合逻辑、RRAM交叉阵列和SRAM等硬件原语上，使整个搜索过程在芯片内完成。

Result: 在22nm工艺下，IMC-MCTS在9x9围棋上仅消耗~60mW，相比CPU能效提升96倍，相比H100 GPU能效提升65-2059倍；竞技水平在采样不确定度内达到与Pachi-UCT和Michi-C等开源围棋引擎相当的欧洲围棋联合会等级分。

Conclusion: 证明了存内计算也能高效执行不规则的多阶段算法，且同一硬件平台可运行覆盖四个AI领域的八种应用，展示了IMC-MCTS作为一种通用、低功耗AI决策方案在边缘部署中的潜力。
...
### [88] [Magnetic Tunnel Junctions for Timekeeping in Intermittent Computing Systems](https://arxiv.org/abs/2607.23000)
...
TL;DR: FLINT是一种利用“破损”磁性隧道结（MTJ）的随机保持失效来测量时间的计时器，相比传统电容器计时方案，能以更低能耗和更小面积实现更宽的时间测量范围，且不会随器件老化而偏移。


<details>
  <summary>Details</summary>
Motivation: 无电池间歇性系统的电源故障会擦除计时状态，导致传感、调度和协调出错。现有基于电容器放电的计时方案存在两大问题：电容器需按最长测量间隔设计（导致范围、能量和面积同步增长），且反复充放电循环会降低电容，导致老化后的估计偏差。

Method: FLINT利用“破损”磁性隧道结（MTJ）阵列的随机保持失效来读取经过的时间。通过工程化设计使MTJ可预测地失去状态，其中衰减时间尺度由器件几何结构固定，读取能耗与测量间隔无关且不随器件老化漂移。

Result: 基于21个制造MTJ验证阵列模型，在真实器件轨迹驱动仿真中：跟踪超过15分钟停机时间误差在10%以内，仅消耗1.03μJ能量，面积小于0.1mm²——相比现有技术，范围提升9.2倍，能耗降低11倍；可扩展至更长间隔且不增加成本；一年部署中调度错误比老化电容器时钟少16-52倍。

Conclusion: FLINT通过新颖的MTJ随机保持失效机制，克服了传统电容器计时方案在范围、能耗、面积和老化漂移方面的根本性局限，为无电池间歇性系统提供了一种更优的计时解决方案。
...
### [89] [Decoding the Skew: Distribution-Aware MoE Inference with Adaptive Kernel Dispatch](https://arxiv.org/abs/2607.23099)
*En-Ming Huang,An-Cheng Chang,Bai-Cheng Jeng,Shih-Hao Hung,H. T. Kung*

Main category: cs.AR

TL;DR: 本文提出DA-MoE，一种面向MoE推理的分布感知调度框架，通过离线建模路由分布与在线匹配最优融合内核，无需CPU-GPU同步，在DeepSeek-V3和Kimi K2上分别提升1.16X和1.29X的融合MoE延迟。


<details>
  <summary>Details</summary>
Motivation: 现有MoE推理系统使用静态token数桶来选择融合内核，忽略了影响tile填充、内存复用和内核效率的每个专家的路由分布，导致次优性能。

Method: 提出分布感知框架，包含Effective Experts指标和基于Dirichlet的逆向建模过程以生成可控路由分布；设计DA-MoE运行时，将实时路由直方图匹配离线调优分布，选择近最优融合内核，无需CPU-GPU同步。

Result: 在HumanEval-X服务轨迹上，DA-MoE在DeepSeek-V3上提升融合MoE延迟1.16X，在Kimi K2上提升1.29X，峰值加速分别为1.40X和1.56X。

Conclusion: 融合内核的最优选择依赖于路由分布倾斜度和token数量，DA-MoE通过分布感知的运行时调度能有效提升MoE推理性能。
...
### [90] [Characterizing Warp Divergence from Pascal to Blackwell](https://arxiv.org/abs/2607.23402)
*Alpin Dale*

Main category: cs.AR

TL;DR: 本文通过跨代（Pascal至Blackwell）NVIDIA GPU的微基准测试、硬件计数器和静态分析，验证了线程束发散的性能代价模型：发散路径数k对应的执行时间为T(k)≈sk，效率为32/k，且该模型在ITS引入前后保持一致。同时揭示了编译器重建汇聚机制从SSY/SYNC指令栈到屏障寄存器的演变，以及Blackwell引入的两级汇聚屏障分类、统一分支指令和显式部分掩码同步等新特性。


<details>
  <summary>Details</summary>
Motivation: Volta引入独立线程调度（ITS）后，业界普遍认为NVIDIA GPU以固定方式处理线程束发散。本文旨在检验这一假设，通过跨代（Pascal作为ITS前基线，涵盖Ampere、Hopper以及数据中心和消费级Blackwell）的系统性实验，分离稳定行为与架构变化，揭示发散性能模型的真实特征。

Method: 采用三种互补方法：（1）周期精确的微基准测试，测量不同发散路径数下的执行时间；（2）硬件计数器采集，分析线程束执行效率与占用率的关系；（3）编译器生成的SASS静态分析，研究重建汇聚机制的指令级变化。同时在Blackwell上进行受控比特翻转实验，验证新屏障分类的实际运行时效果。

Result: （1）所有测试代中，发散性能代价线性于路径数k，T(k)≈sk，无超线性重建惩罚；线程束执行效率为32/k，与占用率无关；谓词化可消除序列化代价。（2）该行为在Pascal上同样成立，表明其早于ITS。（3）编译器重建机制发生了显著变化：Pascal使用每线程束SSY/SYNC指令栈，后续代使用屏障寄存器指令。（4）延迟重建（超出直接后支配点）从Ampere的29例降至Blackwell的2例。（5）Blackwell引入两级汇聚屏障分类、统一分支指令和显式部分掩码线程束同步，但比特翻转实验表明新屏障分类是静态编译器分类，无可观察的运行时效果。

Conclusion: 线程束发散在NVIDIA GPU上具有稳定且可预测的性能代价模型，即使ITS和控制流ISA及重建汇聚机制持续演进，该模型保持不变。Blackwell的架构创新（两级屏障分类、统一分支指令、部分掩码同步）代表了NVIDIA在汇聚机制上的发展方向，但发散的性能模型本质未变。
...
### [91] [CircuitWeave: Topology-Behavior Alignment for Executable Multimodal RTL Generation](https://arxiv.org/abs/2607.23523)
*Jiahao Feng,Haiyan Qin,Zhiwei Xie,Wang Kang*

Main category: cs.AR

TL;DR: CircuitWeave是一个基于契约的多模态框架，通过从原理图中提取拓扑契约、从文本中提取行为契约，融合为电路契约后生成RTL代码。在5,000个可执行验证包上训练，在VerilogEval-Human和RTLLM基准上分别达到46.60%和40.00%的pass@1，相比纯文本基线显著提升。


<details>
  <summary>Details</summary>
Motivation: 纯文本LLM从自然语言规范生成RTL时，文字描述隐式表达了连接性、寄存器边界和状态输出关系，即使接口和周期级行为已明确指定。原理图可以显式表达这些结构关系，从而补充文本传达的行为约束。但简单添加图像存在融合难题：直接多模态解码无法显式分离文本和原理图的证据角色，也无法在代码生成前暴露缺失和冲突的约束。

Method: CircuitWeave提出契约（contract）机制：从原理图提取"拓扑契约"（记录连接性等结构信息），从文本提取"行为契约"（记录行为约束），将两者融合为"电路契约"（序列化对应关系、缺失证据和冲突）。RTL仅从该融合契约生成。采用联合目标函数同时监督契约提取、序列化融合、契约条件RTL生成，以及从参考RTL反向重建覆盖的契约字段。构建5,000个可执行验证包，使用Qwen + LoRA进行微调。

Result: 在VerilogEval-Human上：pass@1=46.60%, pass@5=61.49%, pass@10=65.39%，分别比相同微调的无原理图基线高8.46、5.85和2.57个百分点。在RTLLM上：pass@1=40.00%, pass@5=48.00%, pass@10=52.00%，每个截点比纯文本基线高2个百分点。

Conclusion: CircuitWeave通过契约介导的多模态融合方法，有效结合了原理图的结构显式性和文本的行为约束，在RTL生成任务上显著优于纯文本基线，验证了分离证据角色、显式处理缺失和冲突约束的有效性。
...
### [92] [PIMID: A Full-System Simulator with Intricacy and Diversity for Processing-in-Memory](https://arxiv.org/abs/2607.24196)
*Yuan He,Masaaki Kondo,Galen M. Shipman,Jered B. Dominguez-Trujillo,Shigeki Tomishima,Kazi Asifuzzaman*

Main category: cs.AR

TL;DR: PIMID是一个全系统仿真器，支持共享内存和消息传递两种执行模型，横跨11种存储技术，可将处理元件置于存储层次的任何位置，并提供端到端的时间和能耗分析。实验表明存储技术对执行时间影响超一个数量级，规则内核随PE数量超线性扩展，图遍历存在消息传递通信墙，且卸载仅在带宽类存储器上以时间换能量。


<details>
  <summary>Details</summary>
Motivation: 现有PIM仿真器各自覆盖设计空间的一部分（单一存储技术、固定PE层级、单一执行模型、止步于器件边界），缺少一个统一的工具来全面探索PIM设计空间。

Method: 设计并实现了PIMID，一个执行驱动和迹线驱动的全系统仿真器。它同时支持共享内存（OpenMP）和消息传递（MPI）执行模型，涵盖11种存储技术（7种DRAM标准、SRAM、3种非易失性存储器），可将PE置于从子阵列到逻辑裸片的任意位置，支持PE数量和核心模型保真度的扫描，基于测量拥塞定价内存储网络，通过单进程主机-设备协同仿真解析端到端时间和能耗。

Result: 存储技术单独导致执行时间变化超一个数量级；最佳主机主存并非最佳PIM衬底；规则内核随PE数量超线性扩展（内存储带宽与计算共扩展）；消息传递下的图遍历遇到共享内存不存在的集体通信墙；在全系统范围内，仅在带宽类存储器上卸载以时间换能量；共享内存卸载在HBM3上节省能耗，16核主机保持所有端到端时间收益。

Conclusion: PIMID填补了现有PIM仿真器的关键空白，为PIM设计空间探索提供了统一的、可扩展的全系统仿真平台，其插件接口允许随PIM技术演进通过标准化YAML规范添加新引擎和模型。
...
### [93] [DICE: Detailed Inter-Chiplet End-to-End PHY Modeling for Accurate Chiplet Simulation](https://arxiv.org/abs/2607.24221)
...
### [94] [A Heterogeneous Neural Network Accelerator for End-to-End Multitask RF Signal Recognition](https://arxiv.org/abs/2607.24669)
...
### [95] [Semalith v1.4: A Calibrated 184M Safety Classifier Achieving State-of-the-Art Prompt-Injection Detection at 44x Fewer Parameters than Llama-Guard-3-8B](https://arxiv.org/abs/2607.22545)
*Tejasvi C. Addagada*

Main category: cs.LG

TL;DR: Semalith v1.4是一个184M参数的DeBERTa-v3-base分类器，能在单次前向传播中同时进行提示注入、一般危害和金融监管合规的三轴安全分类。在22个保留基准测试中，以44倍更少的参数在7/7提示注入评估和11/18总体基准上击败Llama-Guard-3-8B，对208个良性代理提示的FPR为0.000。


<details>
  <summary>Details</summary>
Motivation: 金融服务和代理场景中需要同时处理提示注入、监管合规和一般危害的安全分类器，但现有开源防护栏无法在单次推理中完成这一组合任务。

Method: 基于DeBERTa-v3-base的184M参数分类器，采用22类分类头（BENIGN、9种提示注入子类型、一般危害、11个BFSI标签）和4类辅助超类头，使用联合加权损失训练。在从49个公共来源挖掘的76,204行语料上训练，通过SHA-1去重确保21/22基准测试零污染（最大0.22%）。

Result: 在22个保留基准上，Semalith v1.4以44倍更少参数赢得所有7个提示注入评估和11/18总体基准。对208个良性代理提示FPR=0.000（vs Llama-Guard-3-8B的0.063）。在一般危害基准上Llama-Guard-3领先，形成互补性分工。

Conclusion: v1.3推荐用于对话审核部署（ToxicChat F1 0.624）；v1.4推荐用于需要BFSI标签覆盖或对良性代理提示零FPR的场景。
...
### [96] [CORVUS: Context Optimization and Reduction Via Underlying Synchronization for LLM Coding Agents](https://arxiv.org/abs/2607.22711)
*Mingwei Zheng,David OBrien,Siwei Cui,Pardis Pashakhanloo,Rajdeep Mukherjee,Myeongsoo Kim,Sachit Kuhar*

Main category: cs.LG

TL;DR: CORVUS是一种新型轨迹架构，通过将文件读取行为与观察结果解耦，维护文件同步注册表，从而生成更轻量、与代码库状态保持一致的轨迹，减少冗余令牌和推理周期。


<details>
  <summary>Details</summary>
Motivation: 传统的追加式轨迹架构将文件读取行为与观察结果紧密耦合，导致文件快照因代理编辑或并发人类修改而变得过时，引发推理错误和冗余重读，使轨迹臃肿。

Method: CORVUS提出解耦文件读取行为与观察结果，通过维护相关文件的同步注册表，在每个推理周期仅注入当前文件内容，而非追加完整快照。

Result: 在SWE-POLYBENCH_VERIFIED和SWE-BENCH PRO上，平均输入令牌减少9-50%，最终提示缩短15-32%，推理周期减少最多37%，同时保持相当的通过率。

Conclusion: CORVUS通过结构化的轨迹架构改进，有效消除了冗余文件副本和过时快照，提升了LLM编码代理的效率和一致性。
...
### [97] [CausalGate: Causal Importance Distillation for Transformer Module Pruning](https://arxiv.org/abs/2607.22720)
*Kiran Nair,Smriti Regmi,Rodrigue Rizk*

Main category: cs.LG

TL;DR: CausalGate 提出基于因果干预（零化子层输出、KL散度衡量语义损伤）的静态标量门控机制，在不引入运行时开销的前提下实现Transformer推理加速，在多个模型和基准上优于现有动态路由方法。


<details>
  <summary>Details</summary>
Motivation: 现有自适应推理方法依赖隐藏状态相似性、激活幅度等观测性启发式指标来丢弃冗余模块，但这些基于相关性的度量无法捕捉对语义准确性至关重要的非线性结构计算，导致语义损伤评估不准确。

Method: 在校准阶段，CausalGate通过零化单独Attention和MLP子层的输出来进行因果干预，使用最终logit分布的KL散度精确衡量语义损伤；然后将结构重要性层级通过EMA平滑目标和可微分成对排序损失蒸馏为一组静态、轻量化的标量门控，消除运行时路由开销。

Result: 在TinyLlama-1.1B、Qwen2.5-3B和Llama-3.1-8B上的语言建模和常识推理基准测试中，CausalGate一致优于主流的动态路由和跳层基线，将理论计算节省转化为具体的硬件延迟降低，且零运行时开销。

Conclusion: 基于因果干预的重要性度量比基于相关性的启发式指标更可靠地捕捉语义关键计算，且通过蒸馏为静态门控可在不牺牲效果的前提下实现零开销推理加速。
...
### [98] [Progress-conditioned Group Policy Optimization for Long-Horizon Agentic Tasks](https://arxiv.org/abs/2607.22724)
*Kaibing Yang,Guangfeng Cai,Shengtian Yang,Shuo He,Yu Li,Mengyi Liu,Pengwei Chen,Jun Xu,Lei Feng*

Main category: cs.LG

TL;DR: 提出ProGPO方法，通过在零奖励组中基于首次访问新状态的覆盖率为轨迹/步骤分配优势，解决基于组的策略优化在长周期任务中的信用分配陷阱问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于组的策略优化（如GRPO）在困难的长周期任务中面临采样不平衡问题：重复或低效动作主导高概率区域，而有效改变状态的动作采样不足，导致大量全失败组出现，无法提供有效的奖励信号来纠正策略，形成自我强化的信用分配陷阱。

Method: 提出Progress-conditioned Group Policy Optimization (ProGPO)。核心思想：当一个组中所有样本的结局奖励均为零时，使用首次访问观测覆盖度（first-visit observation coverage）来分配相对优势——访问更多新状态的轨迹或步骤获得更高优势，因为到达新观测是任务成功的先决条件。

Result: 在ALFWorld和WebShop两个具有挑战性的智能体基准上，使用Qwen2.5-1.5B/7B-Instruct模型，ProGPO一致优于基于组的基线方法，尤其在困难任务上提升显著。

Conclusion: ProGPO通过利用状态探索信号（新状态覆盖度）作为零结局奖励组中的替代奖励信号，有效打破了信用分配陷阱，提升了长周期任务上的策略优化效果。
...
### [99] [QFedPolyp: A Communication- and Inference-Efficient Federated Learning Framework for Polyp Segmentation](https://arxiv.org/abs/2607.22743)
*Madan Baduwal,Priyanka Paudel*

Main category: cs.LG

TL;DR: QFedPolyp是一个结合量化感知训练与低精度模型通信的联邦学习框架，用于隐私保护的协作式息肉分割。该框架通过8位量化通信将传输成本降低约4倍，同时保持竞争性的分割精度，并实现高达1.5倍的推理加速，适用于实时临床应用。


<details>
  <summary>Details</summary>
Motivation: 集中式深度学习需要医院共享敏感医疗数据存在隐私风险，而联邦学习虽能保护隐私但通过重复传输全精度模型参数导致高通信成本。为解决这一矛盾，提出一种兼顾通信效率和推理效率的联邦学习框架用于协作息肉分割。

Method: QFedPolyp将量化感知训练与低精度模型通信相结合。各医院在私有数据上本地训练轻量级U-Net，同时在训练过程中模拟量化操作。客户端传输量化后的模型参数至中央服务器，服务器使用联邦平均算法进行重建和聚合。在Kvasir-SEG、CVC-ClinicVideoDB、PolypGen和BKAI-IGH NeoPolyp四个数据集上评估。

Result: 全精度联邦训练在Kvasir-SEG上Dice为0.910，在CVC-ClinicVideoDB上为0.930。均匀8位通信将传输成本降低约4倍，同时保持有竞争力的分割精度。量化模型推理速度比全精度模型快达1.5倍。

Conclusion: QFedPolyp实现了隐私保护的协作式息肉分割，降低了通信开销并加快推理速度。生成的轻量级模型适用于实时临床部署。
...
### [100] [Learning to Access Computation: Accessibility Plasticity as a Principle of Adaptive Intelligence](https://arxiv.org/abs/2607.22748)
*Zhaowen Fan*

Main category: cs.LG

TL;DR: 本文提出"可访问性可塑性"原则，将计算能力与计算可访问性区分为两种独立的适应变量，并通过关系级操作实现计算路径的动态重组，在顺序学习任务中证明该方法能在减少能力修改的同时保持任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有神经网络主要在预定义计算结构内通过参数修改进行适应，未能将计算能力与计算可访问性区分为独立的适应变量，缺乏对计算关系层面动态重组的系统化探索。

Method: 形式化定义可访问性可塑性，建立基于关系操作的具体实现机制；提出"重用优先"的适应层级，优先进行可访问性修改，其次才是更昂贵的能力修改和结构变化。

Result: 在顺序学习任务上的概念验证实验表明，可访问性适应能够减少能力修改，同时保持可比较的任务性能。

Conclusion: 可访问性是神经适应中一个独立的维度，为未来动态神经系统中计算关系随环境演化的研究奠定了基础。
...
### [101] [Hierarchical Grading in Large Language Models](https://arxiv.org/abs/2607.22757)
*T. Shaska*

Main category: cs.LG

TL;DR: 本文提出分级大语言模型（GLLMs），通过在Transformer表示空间中引入代数分级结构，将加权标量作用传播到嵌入、自注意力和训练目标中。该框架扩展了分级神经网络和分级Transformer到自回归语言模型，同时保持表达能力、渐近计算复杂度和推理成本不变。其几何基础是不变理论，通过Kempf-Ness泛函表达分级优势，最优分级由两个矩映射的 coincidence 点给出，普通Transformer被视为边界上的半稳定各向同性点。对于分层目标，证明了分级先验与无先验之间的极小极大分离。所有GLLM在训练后均可编译为相同架构和推理复杂度的标准Transformer。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer架构缺乏对表示空间的结构化分级机制，难以自适应地根据目标复杂度和数据分布调整模型内部表示的精细程度。作者希望建立一种代数框架，使Transformer能够自动学习最优的表示分级结构，同时保持与标准Transformer相同的计算复杂度和推理成本。

Method: (1) 在Transformer表示空间引入代数分级（grading），定义加权标量作用；(2) 将分级结构传播到嵌入层、自注意力机制和训练目标函数中；(3) 利用几何不变理论，通过Kempf-Ness泛函和Hilbert-Mumford准则刻画最优分级；(4) 最优分级由两个矩映射的 coincidence 点给出闭式解；(5) 对分层目标建立极小极大分离理论，证明分级先验的样本效率优势；(6) 训练后分级被吸收进学习参数，可编译为标准Transformer。

Result: (1) GLLMs保持与标准Transformer相同的表达能力和计算复杂度；(2) 最优分级构成一个开凸锥，由Hilbert-Mumford准则判定；(3) 最优分级有闭式解，由两个矩映射的 coincidence 点给出；(4) 对分层目标，分级先验相对于无先验在样本效率上存在指数级（以层级数计）的极小极大分离；(5) 分级配置可在训练前通过凸规划求解；(6) 训练后GLLM可无损编译为标准Transformer。

Conclusion: GLLMs提供了一种理论优雅且实用的方法，通过在表示空间中引入可学习的分级结构提升Transformer的表达效率，同时不增加任何推理开销。该框架揭示了普通Transformer实际上是更大分级家族中的一个边界特例，而非最优选择。理论分析表明分级先验在分层目标上具有显著的样本效率优势。
...
### [102] [An Integrated Deep Learning and Statistical Framework for Whole-Network Gene--Environment Association with Leaf Vascular Architecture](https://arxiv.org/abs/2607.22763)
*Geran Zhao,Yangsheng Wang,Xiaotian Dai,Guifang Fu*

Main category: cs.LG

TL;DR: 本文提出了一种集成深度学习和统计的框架，用于从RGB图像中提取完整的叶脉网络结构，并通过半参数稀疏典型相关分析识别与叶脉结构相关的基因-环境相互作用。


<details>
  <summary>Details</summary>
Motivation: 现有基因-环境关联研究主要使用低维汇总性状量化叶脉，丢失了原始图像中的大部分结构信息，需要更全面的方法来表征叶脉结构。

Method: 1) 将完整叶脉结构表示为全网络图像表型；2) 微调EDTER模型提取叶脉网络；3) 结合DiffusionEdge和BSDS500构建新注释数据库；4) 采用SSCCA进行变量选择和关联建模，通过截断潜高斯copula处理稀疏零膨胀数据。

Result: 两个模拟实验验证了框架在复杂度递增情况下的性能；真实杨树数据集识别出三个与叶脉结构相关的显著基因-地理相互作用。

Conclusion: 该框架提供了新的生物学见解，并建立了适用于高维复杂图像表型的通用方法论框架。
...
### [103] [Beyond Shapley: An Influence-Based Data Auditing Pipeline for LLM Alignment and Evaluation](https://arxiv.org/abs/2607.22766)
...
### [104] [DomainPilot: Domain-Level Loss-Guided Two-Stage Data Mixture Optimization for Efficient Language Model Fine-Tuning](https://arxiv.org/abs/2607.22769)
*He Zhang*
...
### [105] [Dementia Etiology Diagnosis via Collaborative Meta Knowledge Enhancement](https://arxiv.org/abs/2607.22770)
...
### [106] [Optimizing Transformer Neural Network for Real-Time Outlier Detection on FPGAs](https://arxiv.org/abs/2607.22786)
...
### [107] [CC-AOS: Cost- and Horizon-Conditioned Amortized Backward Induction for Finite-Horizon Optimal Stopping](https://arxiv.org/abs/2607.22774)
*Tianwei Yu*
...
### [108] [ADVERSARIAL: And-Inverter Graph-Assisted Hardware Trojan Detection At Scale](https://arxiv.org/abs/2607.23882)
...
### [109] [SLA-Constrained Carbon-Aware Routing in Geo-Distributed Serverless Clouds](https://arxiv.org/abs/2607.22806)
...
### [110] [Predicting the Outcome of rTMS Depression Therapy using EEG Signals and CNN](https://arxiv.org/abs/2607.22776)
...
### [111] [LC-SEPLM: long-range contact-supervised adaptation for sequence-only protein representation learning](https://arxiv.org/abs/2607.22777)
...
### [112] [Multimodal Surface EMG Hand Gesture Recognition Using Query-Based Transformers for Prosthetic Control](https://arxiv.org/abs/2607.22779)
...
### [113] [What Softmax Throws Away: Mass-Aware Attention for Evidence Accumulation](https://arxiv.org/abs/2607.22781)
...
### [114] [FMOPF: Latent Flow Matching with Constraint-Aware Interaction Priors for AC Optimal Power Flow](https://arxiv.org/abs/2607.22788)
*Zhilin Huang*
...
### [115] [Multimodal Domain Generalization for Depression Detection: An Attention-Based BiLSTM Network with Domain-Adversarial Training](https://arxiv.org/abs/2607.22794)
...
### [116] [Physically Verifiable Evidence and LLM-Based Reporting for Bearing Fault Diagnosis](https://arxiv.org/abs/2607.22797)
...
### [117] [LithoFormer: A Robust Framework for Stratigraphic Inference via Transformers](https://arxiv.org/abs/2607.22804)
...
### [118] [OrchNAS: Orchestrated Neural Architecture Search Service for Personalised Federated Edge Intelligence](https://arxiv.org/abs/2607.22805)
...
### [119] [From Hybrid Mechanistic--Data-Driven Modeling Toward Neuro-Symbolic AI: What, Why, and How](https://arxiv.org/abs/2607.22811)
...
### [120] [MEMENTO: Memory-Guided Memetic Code-as-Policy Evolution](https://arxiv.org/abs/2607.22832)
...
### [121] [Frustratingly Simple Black-Box Adaptation of Language Models via Logit Bias](https://arxiv.org/abs/2607.22837)
...
### [122] [Same Predictions, Different Reasons: The Effect of Quantization on Model Explanations](https://arxiv.org/abs/2607.22872)
...
### [123] [Spatial Prediction of Soil Microplastics and Organic Matter Using Graph Attention Networks](https://arxiv.org/abs/2607.22875)
...
### [124] [Efficient Learning of Truncated Boolean Product Distributions: Influence to the Rescue](https://arxiv.org/abs/2607.22889)
...
### [125] [Learning from the Descent Direction: Adaptive Gradient Descent under One-Sided Hölder Regularity](https://arxiv.org/abs/2607.22906)
...
### [126] [Beyond Directed Acyclic Graphs: Causal Zeros and Causal Differential Equations](https://arxiv.org/abs/2607.22910)
*Sergei V. Kalinin*
...
### [127] [Hidden Boundary Motion in Transformer Optimization: Function-Space Orthogonalization of Affine Weight and Bias Updates](https://arxiv.org/abs/2607.22927)
...
### [128] [Distribution-Specific Curvature Control with Finite-Sample Guarantees for Open-Weight Safety](https://arxiv.org/abs/2607.22929)
...
### [129] [Spectral-Aware Analytic Class-Incremental Learning for Long-Tailed Distributions](https://arxiv.org/abs/2607.22931)
...
### [130] [Discrepancy-Rounded Fair Bandits with Static and Time-Varying Exposure Floors](https://arxiv.org/abs/2607.22935)
...
### [131] [Verbalized Particle Posterior: Bayesian Inference over Natural Language Hypotheses](https://arxiv.org/abs/2607.22961)
...
### [132] [Learned Interventions in Lean 4 grind](https://arxiv.org/abs/2607.22972)
...
### [133] [Bayesian Complete-Pooling in Cross-Subject Classification for Motor Imagery Electroencephalogram](https://arxiv.org/abs/2607.22980)
*Ethan Davis*
...
### [134] [Finite-Time Analysis of the Natural Policy Gradient in Finite-Horizon Markov Decision Processes](https://arxiv.org/abs/2607.22982)
...
### [135] [Label-free Industrial Fault Detection via Adversarial Inverse Reinforcement Learning: A System for Run-to-Failure Prognostics](https://arxiv.org/abs/2607.22987)
...
### [136] [Recycling computational processes of dynamic programming for combinatorial optimization problems: a reservoir computing approach](https://arxiv.org/abs/2607.23009)
...
### [137] [Mini-batch Noise Lowers Sharpness via Dominant-Subspace Fluctuations](https://arxiv.org/abs/2607.23012)
...
### [138] [All in One: Generative Modeling as Mean-Field Game Design](https://arxiv.org/abs/2607.23026)
*Kun Zhao,Xu Chen*
...
### [139] [Multi-Agent Privacy Game in Federated Learning: A Unified Mean-Field View](https://arxiv.org/abs/2607.23029)
*Kun Zhao,Xu Chen*
...
### [140] [Online Policy Evaluation for MDPs with Dynamic UBSR Measures](https://arxiv.org/abs/2607.23030)
...
### [141] [MixQuant: Adaptive Mixed-Precision Quantization for Large Language Models](https://arxiv.org/abs/2607.23047)
...
### [142] [The Entropic Bound for Transformers: Why Static Rank Fails and Attention-Native Rank Recovers](https://arxiv.org/abs/2607.23050)
*Byeong Hoon Yoon*
...
### [143] [Through the Bottleneck: How Multi-head Latent Attention Separates Content from Position in Language Models](https://arxiv.org/abs/2607.23054)
...
### [144] [Self-Boosting Vision-Language Models with Noisy Student On-Policy Self-Distillation](https://arxiv.org/abs/2607.23125)
...
### [145] [Diffusion-Guided Search via Exponential Tilting (DiffTilt): An Application to Falsification of Safety-Critical Systems](https://arxiv.org/abs/2607.23134)
...
### [146] [Foundation Models and Fine-Tuning: Toward a New Generation of Models for Time Series Forecasting](https://arxiv.org/abs/2607.23146)
...
### [147] [XGRVFL-MV: Residual-Coupled Graph-Embedded Multi-View Random Vector Functional Link Network with FleXi Guardian Loss](https://arxiv.org/abs/2607.23149)
...
### [148] [In-Context Learning as Implicit Policy Gradient](https://arxiv.org/abs/2607.23153)
...
### [149] [Wrong Design Intent Is Worse Than None: A Derangement-Control Diagnosis of Header Conditioning in CAD Program Completion](https://arxiv.org/abs/2607.23191)
*Yang Xiao*
...
### [150] [Domain-Prior-Regularized Graph Modeling for Anomaly Detection in Cyber-Physical Systems](https://arxiv.org/abs/2607.23197)
...
### [151] [Variance-Preserving Orthogonal Selection (VPOS): Greedy Feature Selection via Orthogonal Deflation in PCA Loading Space](https://arxiv.org/abs/2607.23198)
...
### [152] [ParasGB: A Graph Benchmark Suite for Parasitic Estimation on AMS Circuits](https://arxiv.org/abs/2607.23225)
...
### [153] [From Score Learning to Discretized Sampling: An End-to-End Generalization Analysis of Diffusion Models](https://arxiv.org/abs/2607.23226)
...
### [154] [Context-Aware Concept Distillation for Trustworthy Flood Prediction](https://arxiv.org/abs/2607.23237)
...
### [155] [StageGuard: Physiologically Constrained Sleep Staging](https://arxiv.org/abs/2607.23284)
...
### [156] [FILLER: Feature Imputation via Latent Location Exploration and Retrieval](https://arxiv.org/abs/2607.23295)
...
### [157] [AlloBench: Measuring Online Tool Allocation Capability in LLM Agents](https://arxiv.org/abs/2607.23332)
...
### [158] [Training with (Swap) Regret Loss in a Single-Layer Self-Attention Model: A Case Study on the Probability Simplex](https://arxiv.org/abs/2607.23333)
...
### [159] [Neural operator discovery from heterogeneous trajectories](https://arxiv.org/abs/2607.23337)
...
### [160] [Does Graph Compression Preserve Signal Propagation?](https://arxiv.org/abs/2607.23338)
...
### [161] [SPRKD: Effective Knowledge Distillation for Deep Neural Networks via Saddle Region Approximation](https://arxiv.org/abs/2607.23346)
...
### [162] [Exploration of the generative capabilities of Boltzmann machines applied to social systems under the majority rule](https://arxiv.org/abs/2607.23349)
...
### [163] [On the Impossibility of Unbiased and Length-Invariant Policy Optimization with Outcome Rewards](https://arxiv.org/abs/2607.23364)
...
### [164] [Bitcoin Price Direction Prediction via Regime-Aware Multi-Modal Fusion of Social Sentiment and Technical Features](https://arxiv.org/abs/2607.23370)
...
### [165] [Directional Influence Function: Estimating Training Data Influence in Constrained Learning](https://arxiv.org/abs/2607.23388)
...
### [166] [When Can Depth Replace Precision? A Resource Theory of Quantized Neural Computation](https://arxiv.org/abs/2607.23390)
*Mojtaba Soltanalian*
...
### [167] [A Statistical Difference between Single-Layer Learning and Hierarchical Learning in Wide Neural Networks](https://arxiv.org/abs/2607.23397)
*Sumio Watanabe*
...
### [168] [Transfer Learning Architectures for Scalable Multi-Fidelity Bayesian Optimization](https://arxiv.org/abs/2607.23404)
...
### [169] [Blood Pressure Estimation from PPG: A Comparative Study of Direct and ECG-Mediated Deep Learning Pipelines](https://arxiv.org/abs/2607.23406)
...
### [170] [Harmonized Interpretable ECG Waveform Features for Robust Cross-Dataset Clinical Prediction](https://arxiv.org/abs/2607.23412)
...
### [171] [Short-Term Pain for Long-Term Gain: Adaptive Experiment with Post-Commitment Reward Shift](https://arxiv.org/abs/2607.23432)
...
### [172] [PerturbPFN: Probing the Limits of Synthetic Priors in Drug Perturbation Modelling](https://arxiv.org/abs/2607.23447)
...
### [173] [Local Regularization Does Not Characterize Multiclass PAC Learnability](https://arxiv.org/abs/2607.23449)
*Eric Hou*
...
### [174] [Generalization bounds and sample complexity for remaining useful life prediction from complete degradation trajectories](https://arxiv.org/abs/2607.23454)
...
### [175] [Extending Fourier Neural Operators for Modeling Parameterized and Coupled PDEs](https://arxiv.org/abs/2607.23466)
...
### [176] [Learning to Optimize: Joint Routing and Flow Allocation on Sparse Non-Euclidean Networks](https://arxiv.org/abs/2607.23467)
...
### [177] [Sparse Gaussian-Mixture-Model Q-Functions via Hadamard Overparametrization for Online Reinforcement Learning](https://arxiv.org/abs/2607.23474)
...
### [178] [A Multi-stage Constrained Optimization Framework for Data-driven Problems](https://arxiv.org/abs/2607.23480)
*Ye Shi*
...
### [179] [Charging Phase Health Indicators for Battery State-of-Health Estimation: A Systematic Comparison of CC, CV, and Combined Approaches under Cross-Battery Validation](https://arxiv.org/abs/2607.23482)
*Huy Hoang Le,Kim-Anh Nguyen*
...
### [180] [An adaptive multi-fuzzy logic model for diagnosing transformer faults using dynamic weight optimization](https://arxiv.org/abs/2607.23486)
...
### [181] [Learning Sampling Parameters for Diffusion Models](https://arxiv.org/abs/2607.23488)
...
### [182] [Physics-Informed Neural Networks for Discovering Periodic Orbits in the Gravitational Three-Body Problem](https://arxiv.org/abs/2607.23501)
...
### [183] [Impute On-Demand: Adaptive Correlated Time Series Imputation for Changing Environments](https://arxiv.org/abs/2607.23503)
...
### [184] [Topological Data Analysis and Graph-Theoretic Approaches for Tennis Match Prediction](https://arxiv.org/abs/2607.23509)
...
### [185] [Chamaileon: Cross-Context Binder Design with Contextualized Modeling and Mixed Sampling](https://arxiv.org/abs/2607.23518)
...
### [186] [Neonatal Hypoxic-ischaemic Encephalopathy Classification from the EEG and HRV Signals Using a Conformer based Masked Autoencoder](https://arxiv.org/abs/2607.23554)
...
### [187] [Random Forest-Based Prediction of Bone Volume Fraction and Fracture Position from S-Parameters](https://arxiv.org/abs/2607.23563)
...
### [188] [MS-GPT: Rethinking MS/MS De Novo Structure Elucidation as Spectrum-Induced Posterior Querying of a Molecule-Language Model](https://arxiv.org/abs/2607.23607)
...
### [189] [Restoration Flow Matching-Based Channel Refinement and Equalization Correction for MIMO Semantic Communications](https://arxiv.org/abs/2607.23615)
...
### [190] [Optimal Reward Shaping: Autonomous Car Parking Case Study](https://arxiv.org/abs/2607.23617)
...
### [191] [Variational-Ising-Attention (VIA):TailoredAttentionMattersfor Science](https://arxiv.org/abs/2607.23634)
*Rui Wang*
...
### [192] [CALMRec: Causally Aligned Language Memory for Long-Horizon Recommendation](https://arxiv.org/abs/2607.23647)
*Gengyu Zhan*
...
### [193] [DP-IVON-Gradsq: Differentially Private Squared-Gradient Improved Variational Online Newton](https://arxiv.org/abs/2607.23649)
...
### [194] [Breaking the Total Variance Barrier: Sharp Sample Complexity for Linear Heteroscedastic Bandits with Fixed Action Set](https://arxiv.org/abs/2607.23679)
...
### [195] [Extreme Volatility Warning under Label Scarcity via Multi-Source Anomaly Fusion](https://arxiv.org/abs/2607.23682)
...
### [196] [Latent-LoRA: Compact Latent-Space Adapters with Gradient-Free Routing for Continual Learning](https://arxiv.org/abs/2607.23837)
...
### [197] [The Intruder Threshold: A Spectral Law for LoRA Fine-Tuning](https://arxiv.org/abs/2607.23711)
*Peng Xie*
...
### [198] [Outcome-Confounded Local Supervision in On-Policy Distillation](https://arxiv.org/abs/2607.23731)
*Guoqing Ma*
...
### [199] [Understanding Machine Unlearning Through the Lens of Mode Connectivity](https://arxiv.org/abs/2607.23970)
...
### [200] [Source-Free Controlled Adaptation of Teachers for Continual Test-Time Adaptation](https://arxiv.org/abs/2607.23735)
...
### [201] [ACRL: Adaptive Control of Training-Inference Discrepancy for Stable Reinforcement Learning](https://arxiv.org/abs/2607.24062)
...
### [202] [Soft-Constrained Optimization of Latent Space in Variational Autoencoders](https://arxiv.org/abs/2607.23751)
*Ye Shi*
...
### [203] [On the post-hoc Evaluation of PDE Discovery: A Multifaceted Challenge of Scientific Advancement](https://arxiv.org/abs/2607.23753)
...
### [204] [Beyond Aggregate Risk: Role-Stratified Conformal Risk Control for LLM Tool Calls](https://arxiv.org/abs/2607.24343)
...
### [205] [WISERouter: LLM Routing with Workload Budget Constraint](https://arxiv.org/abs/2607.23765)
...
### [206] [What do Reward Models Memorize?](https://arxiv.org/abs/2607.24484)
...
### [207] [A Comparison of Data Augmentation Methods for Training Deep Neural Networks on Synthetic Aperture Sonar](https://arxiv.org/abs/2607.23770)
...
### [208] [Sparse Autoencoders Encode Both Concepts and Functions: The Downstream Geometry of Feature Effects](https://arxiv.org/abs/2607.24645)
...
### [209] [Scale Weight Decay and Train Better](https://arxiv.org/abs/2607.23777)
*Anuj Apte*
...
### [210] [SCTA: An Agentic Framework for Stable and Interpretable Target Gene Discovery from Single-Cell RNA Sequencing](https://arxiv.org/abs/2607.23821)
...
### [211] [DriveDNA: A Large-Scale Multimodal Naturalistic Driving Dataset and Benchmark for Driving Style Identification](https://arxiv.org/abs/2607.23822)
...
### [212] [Covariance Last-Layer Ensembles: Function-Space Diversity for Efficient Uncertainty Quantification](https://arxiv.org/abs/2607.23856)
...
### [213] [Controllable Diversity in Normalization-Based Implicit Ensembles via Softmax-Temperature Modulation](https://arxiv.org/abs/2607.23860)
...
### [214] [XMix: Combating Extremely Noisy Labels via Local Smoothness in Self-Supervised Feature Space](https://arxiv.org/abs/2607.23865)
...
### [215] [A Coulomb Particle Model for Learning Kernel Attention in Transformers](https://arxiv.org/abs/2607.23869)
...
### [216] [Flash-CNNCap: Capacitance Extraction via Image Mapping](https://arxiv.org/abs/2607.23877)
...
### [217] [Physics-Informed Neural Networks for Predicting Nitrous Oxide Flux](https://arxiv.org/abs/2607.23880)
...
### [218] [WorldDiT: A Unified Diffusion Architecture for World and Action Modeling](https://arxiv.org/abs/2607.23909)
...
### [219] [Greedy dynamical meta-learning](https://arxiv.org/abs/2607.23925)
*Aria Yom*
...
### [220] [DECAF: De-Clustering for Adaptive Representational Unlearning](https://arxiv.org/abs/2607.23934)
...
### [221] [Variational Boosting for Physics-Informed Neural Networks](https://arxiv.org/abs/2607.23940)
...
### [222] [Joint Flow Matching for Generator-Consistent Classification](https://arxiv.org/abs/2607.23946)
...
### [223] [Adaptive Data Admission and Retention for Streaming Federated Learning](https://arxiv.org/abs/2607.23987)
...
### [224] [When Should Active RAG Retrieve? A Budget-Aware Evaluation of Utility, Calibration, and Cost](https://arxiv.org/abs/2607.24010)
...
### [225] [Capacity-Aware Deep Learning for Generalizable Traffic Volume Estimation Across Links and Cities](https://arxiv.org/abs/2607.24056)
...
### [226] [Constrained Reinforcement Learning Using Successor Representations](https://arxiv.org/abs/2607.24057)
...
### [227] [Learning Reusable Hybrid Motion Priors for Humanoid Locomotion from Motion Imitation](https://arxiv.org/abs/2607.24083)
...
### [228] [MAPLE: Efficient and Diverse Multi-Alpha Generation for Portfolio Construction](https://arxiv.org/abs/2607.24131)
...
### [229] [An Empirical Study of Feature Selection Granularity](https://arxiv.org/abs/2607.24145)
...
### [230] [Calibrated Tree-Neural Fusion for Fine-Grained Vegetation Community Classification](https://arxiv.org/abs/2607.24160)
...
### [231] [Forecasting the Emergence and Evolution of Crash Hotspots: A Unified Deep Learning Framework for Proactive Traffic Safety](https://arxiv.org/abs/2607.24168)
...
### [232] [Monitoring Post-Disaster Urban Recovery Using High-Resolution SAR Time Series and Unsupervised Learning: Evidence from the 2023 Türkiye-Syria Earthquake](https://arxiv.org/abs/2607.24180)
...
### [233] [Every Client Is an Environment: Federated De-confounding for Spatio-Temporal Forecasting](https://arxiv.org/abs/2607.24218)
...
### [234] [Why does Greedy Search produce Optimal Clustering Outcomes? A Fixed-Core Assignment Theory](https://arxiv.org/abs/2607.24237)
...
### [235] [ML-based Predictive Models for Power Consumption in Virtualised O-RANs](https://arxiv.org/abs/2607.24256)
...
### [236] [KAP: Bridging the Knowledge Selection-Runtime Consumption Gap in LLM Systems](https://arxiv.org/abs/2607.24260)
...
### [237] [Physics-Guided Generative AI for Property-Targeted 3D Porous Media Design](https://arxiv.org/abs/2607.24274)
*Peng Wang*
...
### [238] [MEGA-CL: A Molecular Foundation Model for Generalizable ADMET Prediction through Graph External Attention and Contrastive Learning](https://arxiv.org/abs/2607.24314)
...
### [239] [DynaCalKV: Key-Value Cache Compression via Head Grouping and Adaptive Rank Allocation](https://arxiv.org/abs/2607.24331)
...
### [240] [Unsupervised Graph Representation Learning with Complementary View Alignment](https://arxiv.org/abs/2607.24338)
...
### [241] [Perturbative-NeuSA: A Structured Spectral Framework for Time-Dependent PDEs](https://arxiv.org/abs/2607.24345)
*Xianli Zhu,Jia Yin*
...
### [242] [MobiWave: Dispatch-Oriented Graph Wavelets and Drift-Guided Selective Optimization for Autonomous Fleet Rebalancing](https://arxiv.org/abs/2607.24365)
...
### [243] [MXAttention: Data-Free Optimal Scaling and Pre-Normalization Quantization for MXFP4 Attention](https://arxiv.org/abs/2607.24377)
...
### [244] [K-Survival Means](https://arxiv.org/abs/2607.24405)
*Abdallah Alabdallah*
...
### [245] [Context Is King: How In-Context Specification Shapes the Geometry of Concepts](https://arxiv.org/abs/2607.24425)
...
### [246] [DraftExpert: Expansion-Aware Self-Speculative Decoding for End-Device MoE Inference](https://arxiv.org/abs/2607.24434)
*Dengke Han*
...
### [247] [UNIFUSION: Adapting Autoregressive Language Models into Discrete Diffusion under a Unified Reverse-Rate Objective](https://arxiv.org/abs/2607.24507)
...
### [248] [Physics Transformer: Tailoring Transformer for General PDE Prediction](https://arxiv.org/abs/2607.24513)
...
### [249] [Low-Rank Dependence Decomposition via Accelerated Symmetric Non-negative Matrix Factorization](https://arxiv.org/abs/2607.24518)
...
### [250] [Stress-Testing EEG Foundation Models for Clinical Decoding: Dataset Identity and Targeted Negative Controls](https://arxiv.org/abs/2607.24519)
*Marzieh Zare*
...
### [251] [FlowCTS: On-policy Continuous Trajectory Supervision of Flow Models](https://arxiv.org/abs/2607.24522)
...
### [252] [From Machine Learning to Large-Scale EO Products: Best Practices for Making Maps](https://arxiv.org/abs/2607.24532)
...
### [253] [The K-SCAN Clustering Algorithm](https://arxiv.org/abs/2607.24537)
...
### [254] [EchoBridge: Long-Tail-Aware ECG-Echocardiography Text Alignment for Echocardiography-Derived Cardiac Findings](https://arxiv.org/abs/2607.24553)
...
### [255] [LOCKS: Page-Local Compact Key Summaries for Efficient Long-Context Decoding](https://arxiv.org/abs/2607.24555)
*Junsung Hwang*
...
### [256] [BettiSplit: Topology-Guided Privacy-Aware Split Learning Against Feature Inversion and Gradient Leakage](https://arxiv.org/abs/2607.24556)
...
### [257] [Bit-Accurate FPGA Evaluation of Learned Feature Gating in a Fixed-Point Fourier-Feature Automatic Modulation Classifier](https://arxiv.org/abs/2607.24568)
...
### [258] [Evaluating Fuzz Testing for Reinforcement Learning Agents](https://arxiv.org/abs/2607.24577)
...
### [259] [PYPM-GGD: Pitman-Yor Process Mixture with Generalized Gaussian Density using ADAM](https://arxiv.org/abs/2607.24583)
*Kart-Leong Lim*
...
### [260] [Attribution and Uncertainty Behavior of Learned Residual Gyro Correction for Gyro-Stellar Estimation](https://arxiv.org/abs/2607.24608)
...
### [261] [When Can You Correct Distribution Drift in Temporal Graph Generation? A Sharpening--Drift Tension and an Impossibility for Observation-Based Correction](https://arxiv.org/abs/2607.24662)
...
### [262] [Explainable Reinforcement Learning via Physics-Aware Policy Distillation](https://arxiv.org/abs/2607.24672)
...
### [263] [Causal-TS: A Python Library for Causal Discovery in High-Dimensional and Nonstationary Time Series](https://arxiv.org/abs/2607.24673)
*Mohammad Fesanghary*
...
### [264] [Global Convergence of DGM and PINN Algorithms for Solving Nonlinear PDEs](https://arxiv.org/abs/2607.24726)
...
### [265] [Concept-based Visual Counterfactual Explanations with Diffusion Models](https://arxiv.org/abs/2607.22544)
*Yassine Oueslati,Daniil Kirilenko,Martin Gjoreski,Marc Langheinrich*

Main category: cs.AI

TL;DR: C-VCE 是一种新型扩散框架，通过将分类器内置为概念瓶颈层，无需外部噪声鲁棒分类器即可生成视觉反事实解释，在保持图像质量的同时实现语义级编辑控制。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的反事实解释方法依赖外部分类器，这些分类器需要在噪声图像上可靠工作，导致系统脆弱、部署困难，难以生成鲁棒的解释。

Method: 将分类器直接构建到生成模型中作为概念瓶颈层，通过人类可解释的概念引导反事实生成；添加概率正则化器平衡预测改变与图像保真度；使用梯度掩码限制修改仅作用于最相关区域。

Result: 在 CelebA 等基准上，C-VCE 匹配或超越了现有方法的翻转率，同时生成的图像在视觉上更接近原始输入、失真更少。

Conclusion: 暴露并控制内部概念层是使强大生成模型更易理解、更安全使用的有效途径，C-VCE 为需要"假设"图像的视觉系统提供了实用工具。
...
### [266] [SeT-Diff: Towards Semantic Foundation Models for HPC Telemetry and Time-Series](https://arxiv.org/abs/2607.22548)
*Giovanni B. Esposito,Francesco Antici,Daniele Cesarini,Andrea Bartolini*

Main category: cs.AI

TL;DR: 提出了SeT-Diff，首个基于扩散模型的计算节点遥测时序基础模型，通过将生成过程条件于传感器语义描述，实现数据集结构无关的系统动力学建模，在真实超算数据集上重建MAE达0.0470，并具备零样本排列稳定性与多任务泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有HPC遥测机器学习方法依赖静态的固定位置传感器变量子集，仅针对单一任务，当目标任务变化或传感器指标变更时模型过时失效，缺乏灵活且通用的数字孪生方案。

Method: 基于扩散模型，将每个传感器的语义描述作为生成过程的条件，解耦系统动力学与数据集结构，从而实现与传感器排列无关的时序建模。

Result: 在真实超算数据集上重建MAE为0.0470；零样本排列稳定性下传感器打乱后精度几乎无损；单一预训练模型可有效执行数据插补、预测和虚拟感知，热推理MAE达0.033。

Conclusion: SeT-Diff作为首个计算节点遥测基础模型，通过条件扩散机制实现了对传感器变动的鲁棒性和多任务泛化能力，为HPC系统提供了有效的数据驱动数字孪生方案。
...
### [267] [QFoldAgent: An Autonomous Quantum Optimization Multi-Agent System for Protein Structure Prediction](https://arxiv.org/abs/2607.22549)
*Winson Chen,Yuqi Zhang,Sixu Chen,Nuo Xu,Qiang Guan,Caiwen Ding*

Main category: cs.AI

TL;DR: QFoldAgent是一个闭环多智能体框架，通过设计智能体提出序列条件惩罚项、VQE量子-经典管道优化哈密顿量、反馈智能体利用能量景观诊断和MolProbity验证信号迭代调整惩罚权重，在5残基四面体晶格折叠任务中，将QDockBank基准的中位RMSD从3.64Å降至3.20Å，并将未知序列的结构有效性从87.5%提升至98.7%。


<details>
  <summary>Details</summary>
Motivation: 现有基于晶格的混合量子-经典蛋白质结构预测工作流通常手动固定哈密顿量惩罚权重，且仅在模拟中评估极短片段，缺乏系统化的自动调优机制，导致优化质量不稳定、失败案例较多。

Method: 提出QFoldAgent闭环多智能体框架：(1) 设计智能体根据序列提出条件惩罚系数；(2) VQE量子-经典管道在Qiskit Aer噪声下优化哈密顿量；(3) 反馈智能体利用能量景观诊断和MolProbity验证信号迭代修正惩罚权重；整个过程中RMSD等真实指标对智能体不可见，仅用于最终评估。

Result: 在QDockBank基准上，中位RMSD从3.64Å降至3.20Å，最难目标提升最大；在100个未知序列上，结构有效性从87.5%升至98.7%，87%初始无效案例被恢复；最强控制器在87%的序列上改善了第3轮能量，同时保持96%的Ramachandran偏好几何。

Conclusion: 迭代式智能体控制能够系统性地改善5残基量子折叠场景的优化行为并减少失败案例，表明闭环反馈机制在混合量子-经典蛋白质结构预测中具有显著潜力。
...
### [268] [Same Question, Different Answers: Evaluating LLM Reliability Beyond Accuracy](https://arxiv.org/abs/2607.22554)
*Kazem Faghih,Yize Cheng,Shoumik Saha,Mobina Pournemat,Armin Gerami,Soheil Feizi*

Main category: cs.AI

TL;DR: LLM在不同但等价的表述下，模型答案经常依赖于提示的具体措辞变化，单提示准确性不可靠，但模型通常至少为某个改写版本生成正确答案，通过自我改写策略可部分恢复潜在知识并提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试中LLM表现出高准确率，但不清楚模型在等价的提问方式下能否一致地应用知识，因此需要研究模型对意义保持性改写的稳定性。

Method: 在事实问答和数学推理任务的4个基准测试和13个模型上，使用意义保持性改写观察模型输出的变化，并提出一种简单的自我改写策略来恢复潜在知识。

Result: 模型整体准确率变化不大，但实例级行为不稳定，问答翻转率超过23%；单提示正确性不可靠；模型通常至少能对某个改写版本给出正确答案。

Conclusion: 标准准确率指标可能掩盖实质性的不稳定性，评估等价输入下的一致性才能更清晰地反映LLM的可靠性；自我改写策略可部分利用潜在知识改进性能。
...
### [269] [DeepLens Diagnosis Agent: Agentic Workflow Design Lets a Small Reasoning Model Compete with Frontier LLMs](https://arxiv.org/abs/2607.22555)
*Mahmood Bayeshi,Veysel Kocaman,Muhammed Ali Naqvi,Yigit Gul,David Talby*

Main category: cs.AI

TL;DR: 本文提出DeepLens Diagnosis Agent，一个五阶段诊断管道，以7B小模型为核心，结合检索增强生成（RAG）和结构化流程约束，在诊断推理任务上以极低成本超越Claude Sonnet 4.5和Gemini 3.1 Pro等大模型，证明了流程设计比模型规模更关键。


<details>
  <summary>Details</summary>
Motivation: 现有前沿LLM虽为强通用模型，但在医疗诊断这种多阶段、高不确定性的推理任务中，单次提示（single-shot prompting）往往导致脆弱的诊断推理。小模型虽在标准医疗基准上表现良好（88.2%），但在诊断推理任务中表现不佳（23.99%）。如何通过流程设计而非简单增加参数规模来提升诊断推理能力，是本文的核心动机。

Method: 提出DeepLens Diagnosis Agent，一个五阶段诊断管道：（1）结构化临床信息提取；（2）基于RAG的受控检索；（3）约束性候选诊断生成；（4）显式证据三角验证；（5）可审计的最终决策。核心模型为JSL Medical Small 7B v2小规模医疗推理模型，结合流程约束（harnessing）而非依赖模型规模。

Result: 在915例的DiagnosisArena基准上，DeepLens达到60.14%的top-1诊断准确率，在中小模型中最高。相同模型无管道时仅23.99%，流程设计带来+36个百分点的提升。单例成本0.0072美元（24K tokens），延迟24秒，比Claude Sonnet 4.5（0.0110美元）便宜35-45%，且准确率高出+9.70pp；比Gemini 3.1 Pro（0.0128美元）高出+9.17pp。

Conclusion: 流程约束（harnessing）和结构化推理管道比模型参数规模或API成本更重要；小模型配合精心设计的流程可以在诊断推理任务上超越大型模型。管道产生的结构化中间产物支持逐阶段审查和错误定位，适用于高 stakes场景中可追溯性、可复现性和可审计性要求。
...
### [270] [MIITA: Memory-Induced Inference-Time Adaptation for Continual Learning with Small Language Models](https://arxiv.org/abs/2607.22556)
*Dong Li,Yanchi Liu,Xujiang Zhao,Wei Cheng,Zhengzhang Chen,Xintao Wu,Zhong Chen,Chen Zhao,Haifeng Chen*

Main category: cs.AI

TL;DR: MIITA是一种面向存储受限场景的持续学习方法，通过存储监督经验的修正方向原型并结合语义锚点，在推理时利用语义和不确定性线索检索，经门控临时隐状态适配实现非破坏性知识复用，无需更新骨干网络、扩展提示或测试时反向传播，有效缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在资源受限部署中需持续适应新需求，但直接更新有限参数空间会导致灾难性遗忘。现有基于记忆的方法依赖大语言模型的海量存储和强上下文推理能力，小语言模型难以直接应用。

Method: 1) 将监督经验存储为紧凑的修正方向原型，附带语义锚点；2) 推理时基于语义和不确定性线索检索相关方向；3) 通过门控临时隐状态适配应用检索到的方向，无需骨干网络更新、提示扩展或测试时反向传播。

Result: 在多种有监督持续学习设定下，MIITA在固定存储预算内一致提升最终性能并有效缓解遗忘。

Conclusion: MIITA通过非破坏性的推理时适配机制，为存储受限场景下的小语言模型持续学习提供了一种有效方案，本地理论分析也证实了其与一阶梯度的关联性、不确定性引导检索和方向覆盖能力。
...
### [271] [Codifying the Judge: Scalable Evaluation via Program Distillation](https://arxiv.org/abs/2607.22561)
*Tzu-Heng Huang,Shengqi Qiu,Frederic Sala*

Main category: cs.AI

TL;DR: 本文提出PAJAMA系统，通过将LLM评估逻辑蒸馏为程序化委员会（programmatic judges），实现低成本、高透明度的自动化评估，在匹配13B LLM评估性能的同时，显著降低API成本并提升效率。


<details>
  <summary>Details</summary>
Motivation: LLM-as-a-judge评估范式存在高成本、高延迟和决策不透明的问题，限制了其可扩展性和可靠性，需要更高效、透明的替代方案。

Method: 提出程序蒸馏（program distillation）方法，将LLM的决策逻辑蒸馏为可执行的程序化评判者（programmatic judges）委员会，并构建PAJAMA系统：合成程序作为评判者、聚合决策形成联合裁决、引入降级机制（fallback mechanism）将低置信度案例选择性提升至LLM处理。

Result: 在5个数据集和4个模型家族上，程序化评判者匹配了13B大小LLM评判者的性能；PAJAMA系统在准确率和吞吐量上均提升，推进了帕累托前沿；在RewardBench上，基于程序化评判者蒸馏的奖励模型以两个数量级更低的API成本超越了基于专有LLM标签训练的模型。

Conclusion: 程序蒸馏是LLM-as-a-judge的有效替代方案，兼具透明度、可编辑性及低成本优势，且在评估和奖励信号生成上均表现出色。
...
### [272] [SF-AMS: Strategic Forgetting for Structured Memory in LLM Agent](https://arxiv.org/abs/2607.22562)
*Ning Yang,Siqi Li,Miaoxin Shen,Yuan Zhou,Meng Zhang,Tong Li,Haijun Zhang*

Main category: cs.AI

TL;DR: 提出SF-AMS框架，通过效用驱动的记忆重要性建模替代静态检索和启发式衰减，在长上下文Agent推理中实现紧凑高效用记忆，在多跳推理、时序推理和开放域任务上超越现有最强基线。


<details>
  <summary>Details</summary>
Motivation: LLM Agent中长上下文依赖带来的冗余和无关信息会降低多步推理性能，现有方法依赖静态检索和启发式衰减无法有效维护紧凑高效用记忆。

Method: SF-AMS框架：1) 效用驱动的生存机制，根据使用冗余和时序信号更新记忆重要性，诱导层级记忆结构；2) 复合重要性评分，融合语义和实体级信号提升检索鲁棒性。

Result: 在LoCoMo和LongMemEval-s上超越LightMem、MemO、A-Mem等基线。Qwen2.5-7B多跳推理F1提升+9.65，GPT-4o-mini时序推理F1提升+6.91，开放域任务F1提升+6.53。

Conclusion: 将记忆重要性建模为动态效用信号对可靠的长上下文推理至关重要，SF-AMS表现出跨骨干网络的强泛化能力。
...
### [273] [ParBench: A Benchmark for Reliable Evaluation of LLM Parallel Code Translation](https://arxiv.org/abs/2607.22588)
*Samyak Jhaveri,Erel Kaplan,Tom Yotam,Le Chen,Tomer Bitan,Niranjan Hasabnis,Gal Oren*

Main category: cs.AI

TL;DR: ParBench是一个用于评估大语言模型在并行API翻译（如CUDA、OpenMP、OpenCL等）中语义保真度的基准测试框架，通过内核级翻译、声明式规范和AST驱动的源增强，揭示了现有LLM在并行代码迁移中的方向不对称性、多文件协调、API适配不完整和对源扰动鲁棒性不均等障碍。


<details>
  <summary>Details</summary>
Motivation: 现代计算密集型软件需要跨多种并行编程API（CUDA、OpenMP、OpenCL、OpenMP target offload）进行迁移，LLM和自主编码智能体被越来越多地用于此任务，但缺乏可靠的方法来衡量它们是否保留了底层的并行语义（如线程索引、同步、内存管理、主机-设备协调等），从而保证翻译的行为有效性。

Method: ParBench采用内核中心基准测试框架：通过声明式基准规范固定构建、运行和验证基础设施，仅要求模型翻译计算内核；覆盖多种开源HPC套件及跨API翻译方向；引入AST驱动的、意图保持的、基线验证的源增强方法，以区分稳健翻译与表面形式的记忆。

Result: 对最先进的开源和专有LLM的评估显示，存在持续性的并行代码翻译障碍，包括方向不对称性、多文件协调困难、API适配不完整以及对源级扰动的鲁棒性不均。

Conclusion: 当前的LLM在并行API翻译中仍面临根本性挑战，ParBench为系统评估这些能力提供了可执行、可复现的标准化框架。
...
### [274] [Synthetic Scenario Generation for Evaluation of Industry 4.0 Agents](https://arxiv.org/abs/2607.22563)
*Sagar Chethan Kumar,Rohith Kanathur,Dhaval Patel,Kaoutar El Maghraoui*

Main category: cs.AI

TL;DR: 本文针对工业智能体基准测试中场景生成效率低、覆盖范围有限的问题，提出了ScenarioGeneratorAgent——一种基于IEC标准的合成场景生成流水线，在Smart Grid Transformer资产类别上实现了8倍加速，且场景质量与人工编写相当。


<details>
  <summary>Details</summary>
Motivation: 现有工业智能体基准（如AssetOpsBench）依赖人工编写场景，覆盖资产类别有限，难以高效扩展。

Method: 提出ScenarioGeneratorAgent流水线，包含证据支撑的资产画像构建、覆盖感知的场景预算分配、混合验证-修复循环（确保模式有效性、工具可达性、物理合理性、标准对齐和去重），并采用两级缓存、并行焦点组生成、线程池卸载、批量LLM调用和早期拒绝过滤等优化策略。

Result: 在Smart Grid Transformer场景生成中，优化后流水线端到端运行时间减少8倍（50个场景），复合质量评分74.2±1.9，与未优化基线的73.8±3.0相当。

Conclusion: 基于标准的合成场景生成能够高效扩展工业智能体基准，同时保持场景质量。
...
### [275] [PTStore (Prefix Tensor Store): Distributed Prefix Caching and Replication for High Throughput Inference Serving](https://arxiv.org/abs/2607.22648)
...
### [276] [Loss-Aware Feature-Map Pruning in Convolutional Neural Networks Using Multi-Armed Bandits](https://arxiv.org/abs/2607.22564)
...
### [277] [DSTFView: Multi-View Cloud-Edge Workload Forecasting with Dual-Input Spatio-Temporal-Frequency Modeling](https://arxiv.org/abs/2607.22565)
...
### [278] [MedLoCoMo: A Long-Context Multi-Session Medical Dialogue Benchmark for Large Language Models](https://arxiv.org/abs/2607.22566)
...
### [279] [Keyword Matters: Unveiling the Energy Sensitivity of On-Device LLM Prompting](https://arxiv.org/abs/2607.22568)
...
### [280] [Execution-Grounded Security Testing for Coding Agents in Software Engineering Pipelines](https://arxiv.org/abs/2607.22569)
...
### [281] [Reference Feature Atlases for Mechanistic Auditing of Language Models](https://arxiv.org/abs/2607.22570)
*Rui Wu,Tong Che*
...
### [282] [SCAIR: Schema-Conditioned Agentic Iterative Reasoning for Enterprise Knowledge Graphs](https://arxiv.org/abs/2607.22571)
...
### [283] [Schema-Aware Localisation (SAL): Live Schema Grounding and Hallucination Validation for Oracle NL2SQL](https://arxiv.org/abs/2607.22572)
...
### [284] [PhononBench-MP40: a spectrum-resolved benchmark dataset for phonon stability](https://arxiv.org/abs/2607.22573)
...
### [285] [Too much evidence, too little time: From text to actionable recommendations through multi-objective evidence reasoning](https://arxiv.org/abs/2607.22574)
...
### [286] [Temporal Context Reinstatement Drives Episodic-Like Order Memory in Long-Context Language Models](https://arxiv.org/abs/2607.22575)
...
### [287] [cMoLLM at Scale: Horizontal Scaling Laws for Mixture-of-LLMs](https://arxiv.org/abs/2607.22577)
...
### [288] [HeraSys: Collaborative Serving of Multiple LLM Workflows via Fine-Grained End-to-End Optimization](https://arxiv.org/abs/2607.22578)
...
### [289] [Multi-Objective Structured Pruning of LLMs for Latency and Model Size Optimization](https://arxiv.org/abs/2607.22583)
...
### [290] [Source-Aware Reranking for Retrieval-Augmented Generation: A Reliability Prior Approach](https://arxiv.org/abs/2607.22584)
...
### [291] [The Scaffold Effect in Coding Agents: Harness Choice as a Hidden Variable in Coding-Agent Evaluation](https://arxiv.org/abs/2607.22585)
...
### [292] [MM-ShiftKV: Decode-Aware Prefill-Stage KV Selection for Multimodal Large Language Models](https://arxiv.org/abs/2607.22586)
...
### [293] [TriSP: Tri-Signal Structured Pruning for Large Language Models](https://arxiv.org/abs/2607.22587)
...
### [294] [Lexical discovery in unknown environments orchestrated by Large Language Models](https://arxiv.org/abs/2607.22591)
...
### [295] [Structure Over Scale: Schema-Constrained Causal Graphs for RAG](https://arxiv.org/abs/2607.22592)
...
### [296] [xMIx: High-Performance Serving-Time Platform for Mechanistic Interpretability Apps](https://arxiv.org/abs/2607.22595)
...
### [297] [An Agentic Orchestration of Atomistic Simulations](https://arxiv.org/abs/2607.22596)
...
### [298] [HyCE-RAG: Hypergraph Chain-of-Evidence Retrieval-Augmented Generation for Explainable Multi-hop Question Answering](https://arxiv.org/abs/2607.22597)
...
### [299] [Differencing the Diffusion Trajectory toward Uncertain Components for Time Series Forecasting](https://arxiv.org/abs/2607.22599)
...
### [300] [Chart Deception in Vision-Language Models: From Vulnerability to Mitigation](https://arxiv.org/abs/2607.22600)
...
### [301] [DeepLook: Deeper Thinking with Lookahead](https://arxiv.org/abs/2607.22602)
...
### [302] [Group Preference Collapse in Personalized Multimodal Large Language Models](https://arxiv.org/abs/2607.22603)
...
### [303] [Evaluating LLMs as Interpretable Controllers for Dynamical Systems](https://arxiv.org/abs/2607.22609)
...
### [304] [Tokengeist: Multi-Turn Attribution Tracing in Agentic Conversations](https://arxiv.org/abs/2607.22610)
...
### [305] [Decentralized Granular Access Control for Agentic AI Systems in Critical Infrastructure](https://arxiv.org/abs/2607.22611)
...
### [306] [DynaResize: Runtime GPU Reallocation for Disaggregated LLM Post-Training](https://arxiv.org/abs/2607.22614)
...
### [307] [Opti-Q: A Constraint-Based Optimization Framework for Multi-LLM Question Planning](https://arxiv.org/abs/2607.22621)
...
### [308] [CHS-SQL: A Text-to-SQL approach based on Confidence-Guided Heuristic Search Schema Linking process](https://arxiv.org/abs/2607.22624)
...
### [309] [TokenMem: Faithful Knowledge Injection for Frozen LLMs](https://arxiv.org/abs/2607.22625)
...
### [310] [Masked Distillation: Internalizing the Chain-of-Thought in Language Models](https://arxiv.org/abs/2607.22629)
...
### [311] [VlogReward: Learning Multi-Dimensional Evaluation for Vlog Editing](https://arxiv.org/abs/2607.22632)
...
### [312] [Evolving from Lessons: Skill-Augmented Table Graph Reasoning for Operation-wise Table Question Answering](https://arxiv.org/abs/2607.22633)
...
### [313] [PRESTO: Prefix-Aligned Tree Drafting for Diffusion Speculative Decoding](https://arxiv.org/abs/2607.22634)
...
### [314] [CallBench: A Benchmark for Dual-Goal Coordination in Phone Call Assistants](https://arxiv.org/abs/2607.22635)
...
### [315] [Answering Path Queries under Linear and Guarded Existential Rules](https://arxiv.org/abs/2607.22636)
...
### [316] [Fast Cross-Scenario Adaptation of CSI Models via Channel Conditional Parameter Generation](https://arxiv.org/abs/2607.22637)
...
### [317] [TRACE: Business Rule-Grounded Reasoning Curriculum for Knowledge-Preserving Parametric Tool Retrieval in Enterprise LLMs](https://arxiv.org/abs/2607.22639)
...
### [318] [CRAFT: Learn the Schema, Execute the Plan](https://arxiv.org/abs/2607.22642)
...
### [319] [Reason Before You Retrieve: Agentic Planning for Multi-modal RAG](https://arxiv.org/abs/2607.22643)
...
### [320] [DocHRL: A Hierarchical Reinforcement Learning Framework for Cost-Optimised Document Classification](https://arxiv.org/abs/2607.22644)
...
### [321] [Extracting Algorithms in Pre-trained LLMs: A Case on Hidden Markov Models](https://arxiv.org/abs/2607.22646)
...
### [322] [STAIF: A Stage-wise Optimization for Complex Instruction Following](https://arxiv.org/abs/2607.22649)
...
### [323] [ARdena: Scenario-driven control of real-time LLM agents](https://arxiv.org/abs/2607.22651)
...
### [324] [KG2Code: Bridging Knowledge Graphs and Large Language Models via Executable Code for Question Answering](https://arxiv.org/abs/2607.22652)
...
### [325] [Do Language Models Converge to Themselves? Recursive Self-Refinement as Textual Relaxation](https://arxiv.org/abs/2607.22653)
...
### [326] [MINT-V2X: A Mobility-Integrated Network Trajectory Dataset for Predictive Resource Management](https://arxiv.org/abs/2607.22654)
...
### [327] [EventOD: Event-Aware OD Flow Generation via LLM-Guided Semantic Modulation](https://arxiv.org/abs/2607.22655)
...
### [328] [StanceBench: A Benchmark for Audio LLM-Based Interpersonal Stance Evaluation from Speech](https://arxiv.org/abs/2607.22658)
...
### [329] [TRE: Training-Free Hallucination Detection for Diffusion Language Models](https://arxiv.org/abs/2607.22661)
...
### [330] [CuraWeb: Joint Optimization of Quality, Redundancy, and Diversity for Web-Scale Pretraining Data](https://arxiv.org/abs/2607.22662)
...
### [331] [Beyond Block Boundaries: Multi-Block Editing for Diffusion Large Language Models](https://arxiv.org/abs/2607.22663)
...
### [332] [Obliviate: Efficient Unlearning in Recommender Systems](https://arxiv.org/abs/2607.22665)
...
### [333] [Reinforcement Learning for Heterogeneous Sensor Selection in Maritime Surveillance](https://arxiv.org/abs/2607.22667)
...
### [334] [AIR-BENCH Live: An Evolving Safety Benchmark for Foundation Models](https://arxiv.org/abs/2607.22671)
...
### [335] [How LLM Task-Adaptation Reshapes Alignment: A Multi-dimensional Study of Behavioral and Representational Drift](https://arxiv.org/abs/2607.22676)
...
### [336] [DOSA: A Tree-Guided, Self-Regressive Framework for Long Document Structure Analysis](https://arxiv.org/abs/2607.22679)
...
### [337] [A Vocabulary for Multi-Agent Automated Research Systems](https://arxiv.org/abs/2607.22682)
*Bardiya Akhbari*
...
### [338] [Imprompt: A Language Framework for Prompt Programming](https://arxiv.org/abs/2607.22683)
...
### [339] [Co-Harness: Co-Evolving Harnesses and Model Weights for LLM Agents](https://arxiv.org/abs/2607.22688)
...
### [340] [Beyond Sequential Interaction: Benchmarking Parallel Execution and Coordination for GUI Agents](https://arxiv.org/abs/2607.22689)
...
### [341] [LazyMem: Retrieve Broadly, Construct Selectively for Efficient Long-Term Agent Memory](https://arxiv.org/abs/2607.22690)
...
### [342] [HiLLTS: Zero-Shot Hierarchical LLM-Guided Traffic Signal Control for Sustainable Transportation](https://arxiv.org/abs/2607.22691)
...
### [343] [Risk Governance for Generative AI Mental Health Support: A Multi-Turn Safety Architecture](https://arxiv.org/abs/2607.22692)
...
### [344] [Bayesian Repetition Penalty: A Principled Adjacent-Conditional Framework for Reversing Attention Collapse in Autoregressive Language Models](https://arxiv.org/abs/2607.22694)
...
### [345] [PANOPTICON: A PII-Based Assemblage of Naturalistic Output Tokens for Investigating Privacy Leakage Within LLM Context Window](https://arxiv.org/abs/2607.22695)
...
### [346] [Test-Time Coverage: Test-Conditioned Data Curation for Deployment-Aware Learning](https://arxiv.org/abs/2607.22697)
...
### [347] [Similarity All The Way Up: Multilingual Generalization in LLMs Relies on Language-Level Similarity Structures](https://arxiv.org/abs/2607.22699)
...
### [348] [RoleMix: Unifying Sequential and Non-Sequential Features via Semantic Tokenization for Post-Click Conversion Rate Prediction](https://arxiv.org/abs/2607.22700)
...
### [349] [MPR-CiteG: Enhancing RAG with Multi-Portfolio Retrieval and Citation-Grounded Generation](https://arxiv.org/abs/2607.22706)
...
### [350] [SEGRA: Structured Experience-Guided Graph Reasoning Agent for Gremlin Based Question Answering](https://arxiv.org/abs/2607.22713)
...
### [351] [Spatial Reasoning in LLM Game Agents: Impact of Causal Context and Multi-Step Planning](https://arxiv.org/abs/2607.22732)
...
### [352] [Commitment To Cooperation With Self-Negotiated Contracts](https://arxiv.org/abs/2607.22750)
...
### [353] [Disentangling Multi-View Scanning in Mamba for Network Traffic Anomaly Detection](https://arxiv.org/abs/2607.22829)
...
### [354] [Coordinated Networking for On-Device Agent-Augmented Real-Time Communication](https://arxiv.org/abs/2607.22854)
...
### [355] [What Can Be Enforced? A Theory of Certified Runtime Safety for Tool-Using Agents](https://arxiv.org/abs/2607.22868)
*Shawn Ray*
...
### [356] [Physical AI Governance: From Theory to Practice Across Life Cycle](https://arxiv.org/abs/2607.22877)
...
### [357] [How Well Can AI Generate Backlogs from App Mockups?](https://arxiv.org/abs/2607.22902)
...
### [358] [Agent Team Work Zone: An Automated, Persistent Workspace for Long-Lived Coding Agent Teams](https://arxiv.org/abs/2607.22917)
*Shouren Wang*
...
### [359] [SAGE: Safety-First Defense-in-Depth Guardrails for Verified Lifecycle Control of High-Impact Generative AI](https://arxiv.org/abs/2607.22926)
*Mahdi Eslamimehr*
...
### [361] [Design Theater: A Benchmark for Generative UI](https://arxiv.org/abs/2607.22928)
...
### [362] [Let AI Agents Translate Networks, Not Reason About Them](https://arxiv.org/abs/2607.22947)
...
### [364] [Share No More Than the Request Requires: Federated Disclosure for Perspective-Aware AI](https://arxiv.org/abs/2607.22953)
...
### [366] [Stress-testing large language model agents in a robotic chemistry laboratory](https://arxiv.org/abs/2607.23045)
...
### [368] [SymStep: Symbolic Step Verification for Logical Reasoning](https://arxiv.org/abs/2607.23055)
...
### [369] [Structure over Depth: A Single-Block Spatio-Temporal Transformer for Multi-Entity Reasoning](https://arxiv.org/abs/2607.23077)
...
### [370] [Compiler-Grounded Hierarchical Diagnosis for LLM-Based Triton Kernel Optimization](https://arxiv.org/abs/2607.23089)
...
### [372] [SQBench: A Benchmark for Evaluating Task Delivery by Language-Model Agents in Production-Oriented Workflows](https://arxiv.org/abs/2607.23123)
*Summer Sun*
...
### [373] [CachedSearch: Training-Free Cached Exploration for Test-Time Search in Video Diffusion](https://arxiv.org/abs/2607.23159)
...
### [374] [An Ontology for Machine Learning Interatomic Potentials](https://arxiv.org/abs/2607.23219)
...
### [375] [Characterisation of Density-based FM generation methods in the context of Information Fusion](https://arxiv.org/abs/2607.23243)
...
### [376] [CAPT: A Multi-task Continuous Autoregressive Transformer enabling Cross-dataset and Cross-species Transfer for Calcium Population Dynamics](https://arxiv.org/abs/2607.23258)
...
### [377] [SeekJudge: A Practical Reward Framework for Reinforcement Learning in Computer-Use Agents](https://arxiv.org/abs/2607.23263)
...
### [378] [TopoFE: topology-aware LLM-guided Automated Feature Engineering](https://arxiv.org/abs/2607.23286)
...
### [379] [RareLens: Towards End-to-End Rare Disease Care via Aligning Divergent Large Language Model Reasoning](https://arxiv.org/abs/2607.23290)
...
### [380] [Ordered Network Analysis of Epistemic Emotions during Collaborative Problem Solving](https://arxiv.org/abs/2607.23317)
...
### [381] [ESF-Bench: Benchmarking Challenging Slot-Filling Scenarios for Real-World Enterprise Applications](https://arxiv.org/abs/2607.23326)
...
### [382] [Confidently Wrong: Exception Chain Collapse in Frontier LLM Rule Evaluation](https://arxiv.org/abs/2607.23386)
...
### [383] [Key-Interval A*: Accelerating Grid Pathfinding via Structural Abstraction](https://arxiv.org/abs/2607.23393)
*Taiquan Sui*
...
### [384] [Inference-Time Consensus for Mitigating Hidden Behaviors from LLM Fine-Tuning](https://arxiv.org/abs/2607.23394)
...
### [385] [NeurGO: Learning to Generate Elite Candidates for Meta-Black-Box Expensive Optimization](https://arxiv.org/abs/2607.23408)
...
### [386] [Separating Capability from Permission: A Governance Framework for Agentic AI Autonomy Levels](https://arxiv.org/abs/2607.23438)
...
### [387] [Do LLMs Know Their Vulnerable Scenarios?](https://arxiv.org/abs/2607.23496)
...
### [388] [Delegation Intelligence in Deep Search: A Controllable Framework for Disentangled Capability Diagnosis](https://arxiv.org/abs/2607.23524)
...
### [389] [ObsDriveBench: Benchmarking Multimodal Understanding under Adverse Weather with Observability Awareness](https://arxiv.org/abs/2607.23537)
...
### [390] [Verification-Notebook Learning for Source-Aware Multimodal Misinformation Detection](https://arxiv.org/abs/2607.23581)
*Junyuan Tan*
...
### [391] [Are You Still the Agent I Authorized? Earned Authority under a Fixed Ceiling for Evolving Agents](https://arxiv.org/abs/2607.23586)
...
### [392] [Hybrid Advantage Estimation with Unified Critic for VLM Agentic Reinforcement Learning](https://arxiv.org/abs/2607.23605)
...
### [393] [SpecAHD: Localize to Specialize for Automated Heuristic Design in Large-Scale Routing Problems](https://arxiv.org/abs/2607.23676)
...
### [394] [Focus Is All You Need: Adaptive Goal-aware Attention Orchestration for Multi-Agent Graph Systems](https://arxiv.org/abs/2607.23678)
...
### [395] [Compute Globally, Materialize Locally: The Memory Contract of Sparse Event-KV](https://arxiv.org/abs/2607.23693)
...
### [396] [Offline-to-Online Creative Optimization with Generative Models and Adaptive Testing](https://arxiv.org/abs/2607.23696)
...
### [397] [Offline-Online Curriculum RL for Multimodal Reasoning](https://arxiv.org/abs/2607.23700)
...
### [398] [E-Bench: Benchmarking Multi-Step Tool-Use Agents in Real-World Product Scenarios](https://arxiv.org/abs/2607.23722)
...
### [399] [Training Language Models to Cooperate with Inference-Time Controllers](https://arxiv.org/abs/2607.23771)
...
### [400] [From RLVR to RLSVR: Task Transformation Induces Self-Verifiable Rewards for Open-Ended LLM Self-Improvement](https://arxiv.org/abs/2607.23802)
...
### [401] [ACM: Agentic Context Management for Long Horizon Tasks](https://arxiv.org/abs/2607.23809)
...
### [402] [Do Visual Features Improve Other-Initiated Repair Detection? A Dyadic Multimodal Approach](https://arxiv.org/abs/2607.23845)
...
### [403] [Understanding Human-like Solutions in Combinatorial Optimization via Learning and Search](https://arxiv.org/abs/2607.23854)
...
### [404] [Cost-Aware Recovery-Pathway Identification and Bayesian Optimization for Autonomous Materials Discovery](https://arxiv.org/abs/2607.23896)
...
### [405] [GOTS: Greedy Orthogonal Token Selection for High-Resolution Vision-Language Models](https://arxiv.org/abs/2607.23913)
...
### [406] [MemTX: Transactional Belief Commit for Stateful Agent Memory](https://arxiv.org/abs/2607.23929)
...
### [407] [From Cognitive Architectures to Language Agents: A Mechanism-Level Review of Lineage, Convergence, and Migration Gaps](https://arxiv.org/abs/2607.23942)
...
### [408] [DICA: Dual-Indicator Guided Contrastive Alignment in Multimodal Large Language Models](https://arxiv.org/abs/2607.23944)
...
### [409] [EviBack: Search-Agent Reinforcement Learning via Evidence-Constrained Teacher Backoff](https://arxiv.org/abs/2607.23955)
...
### [410] [Grokking on the Weight-Decay Clock: A Rate Hierarchy from Softly Broken Symmetries](https://arxiv.org/abs/2607.23967)
*Taeyoung Kim*
...
### [411] [Plato-Bio: verification-first biological novelty screening with temporal rediscovery and structural benchmarks](https://arxiv.org/abs/2607.23975)
*Stefan G. Creadore*
...
### [412] [Exploring Budgeted Image Classification with Content-Sensitive Resource Allocation](https://arxiv.org/abs/2607.23997)
...
### [413] [Self-Supervised Consistency Enhanced Disentangled Learning for Neural Decoding Generalization in Brain-Machine Interface](https://arxiv.org/abs/2607.24023)
...
### [414] [A Cyclic Adaptation-Generalization Framework with Uncertainty-Guided Self-Paced Learning for Long-Term Brain-Machine Interfaces](https://arxiv.org/abs/2607.24031)
*Jiyu Wei,Di Hong,Zhanjie Zhang,Dazhong Rong,Qinming He,Yueming Wang*
...
### [415] [The Half-Lives of Generative-AI Evidence: A 40-Record Audit, a Claim-Currency Framework, and a Reflexive Case of Frontier-Model-Assisted Research](https://arxiv.org/abs/2607.24032)
*Carlo Iacono*
...
### [416] [Quantum-Inspired Evolutionary Neighborhood Search for Arrival-Departure Track Utilization Adjustment under Short-Term Disturbances](https://arxiv.org/abs/2607.24049)
...
### [417] [Success Is Not Self-Explanatory: Auditing Success Provenance in Agent Evaluation](https://arxiv.org/abs/2607.24054)
...
### [418] [The Cost of Knowing: A Resource-Aware Protocol for Benchmarking Hallucination Beyond Static Leaderboards](https://arxiv.org/abs/2607.24063)
...
### [419] [MiSS: A Logic-Driven Explanation of Minimal Sufficient Coalitions for Point Cloud Classifiers](https://arxiv.org/abs/2607.24074)
...
### [420] [Towards High-Level Semantic Intelligence](https://arxiv.org/abs/2607.24082)
...
### [421] [MemChain: Learning Interpretable Memory Traces for Memory-Augmented LLM Agents](https://arxiv.org/abs/2607.24097)
...
### [422] [Scaling GUI Agents with Visual State Transitions](https://arxiv.org/abs/2607.24112)
...
### [423] [Grading the Narrators: An Isnad-Rijal Framework for Claim-Level Provenance in Multi-Agent Knowledge Systems](https://arxiv.org/abs/2607.24117)
*Ali Zahid Raja*
...
### [424] [A Motion-Aware Vector Quantization Framework with Centroid Reuse for Efficient VLA Inference](https://arxiv.org/abs/2607.24148)
...
### [425] [Agent-UCT: Upper Confidence Bounds Applied to Trees for Agentic Workflow Optimization with Cost-Awareness](https://arxiv.org/abs/2607.24162)
...
### [426] [Falsifiable Commitment Planning for Self-Correcting Web Agents](https://arxiv.org/abs/2607.24167)
...
### [427] [Myopia Prevention and Control 3.0: Artificial Intelligence--Driven Risk Stratification, Proactive Monitoring, and Personalized Intervention](https://arxiv.org/abs/2607.24187)
...
### [428] [Integrating Factual and Normative Industrial Knowledge via Constraint-Aware Graph Attention for Process Plan Recommendation](https://arxiv.org/abs/2607.24213)
...
### [429] [Epistemic Norms for AI Safety and Alignment Research](https://arxiv.org/abs/2607.24243)
*Keivan Navaie*
...
### [430] [Generative Artificial Intelligence (GenAI) to convert images of queuing networks into verifiable simulation models: an open-weight LLM workflow approach](https://arxiv.org/abs/2607.24259)
...
### [431] [From Proprietary to Open-Source: Bridging the Distribution Gap via Multi-Agent Protocol Distillation in Agentic Search](https://arxiv.org/abs/2607.24280)
...
### [432] [Unequal Trips, Unequal Places: Diagnosing and Mitigating Delay Inequity in Autonomous Vehicle Fleet Coordination](https://arxiv.org/abs/2607.24336)
...
### [433] [Simulating Tenant Responses to Energy Policy Interventions with Transaction-Cost-Aware LLM Age](https://arxiv.org/abs/2607.24341)
...
### [434] [Are Prompt Optimizers Blind? Cross-Modal Visual Feedback for Automatic Prompt Optimization](https://arxiv.org/abs/2607.24354)
...
### [435] [Failures Reveal What Metrics Miss: An Evidence-Driven Agent for Recursive Refinement of ECG Classifiers](https://arxiv.org/abs/2607.24419)
...
### [436] [From Execution to Capability: Scientific Experience Consolidation via Procedural Knowledge Synthesis](https://arxiv.org/abs/2607.24459)
...
### [437] [Making Mathematical Knowledge Explainable, Accessible and Interoperable Through Large Language Model Integration](https://arxiv.org/abs/2607.24512)
...
### [438] [Task-Conditional Faithfulness Auditing of Multimodal LLMs for Grid Diagnosis](https://arxiv.org/abs/2607.24539)
...
### [439] [LLM-Assisted Ontology Engineering and Construction of a French Legal Knowledge Graph](https://arxiv.org/abs/2607.24551)
...
### [440] [Hierarchical Group-Conditional Conformal Risk Control for Selective Prediction in Language Models](https://arxiv.org/abs/2607.24562)
...
### [441] [TRACE-CTI: Auditable Post-Extraction Governance of TTP Claims with Knowledge Graphs](https://arxiv.org/abs/2607.24563)
...
### [442] [DSCH-Loss: A Dynamic Semantic Channel Objective for Deep Semantic Hashing](https://arxiv.org/abs/2607.24567)
...
### [443] [LLM-SoccerArena: Benchmarking LLMs on Real-World Predictions in Sports](https://arxiv.org/abs/2607.24573)
...
### [444] [SIREN: Towards End-to-End Extreme-Weather Early Warning with Experience-Grounded LLM Agents](https://arxiv.org/abs/2607.24588)
...
### [445] [Artificial Intelligence and Innovation Ecosystem: Evolutionary Developments, Challenges, and Future Directions](https://arxiv.org/abs/2607.24589)
...
### [446] [Efficiency Matters in Autonomous Research](https://arxiv.org/abs/2607.24647)
...
### [447] [Reason-Mediated Behavioral Models for Auditing LLM Social Simulators](https://arxiv.org/abs/2607.24649)
...
### [448] [Eviction as Estimation: A Fixed-Lag Smoothing View of Test-Time Memory, and When Measuring Beats Accumulating](https://arxiv.org/abs/2607.24667)
...
### [449] [ERUnderstand: Evaluating Vision-Language Models on Structured ER Diagrams](https://arxiv.org/abs/2607.24707)
...
### [450] [$g$MAGNUS: Fast SpGEMM on GPUs for Irregular Matrices via Hierarchical Multisplit](https://arxiv.org/abs/2607.22866)
*Jordi Wolfson-Pou,Ahmed Helal,Fabrizio Petrini*

Main category: cs.DC

TL;DR: gMAGNUS是一种针对GPU上不规则稀疏矩阵乘法（SpGEMM）的新算法，通过对重型行进行行内重排序，将中间结果细分为可完全在局部内存中累积的独立块，结合新颖的外积与分层多分割操作，在Intel Ponte Vecchio和NVIDIA H200上相比五种领先算法实现了1.81至7.62倍的几何平均加速，且核心内核性能接近理论峰值。


<details>
  <summary>Details</summary>
Motivation: 不规则稀疏矩阵中大量存在的重型行（heavy rows）在SpGEMM计算时会产生大量中间乘积，迫使局部内存累加器溢出到全局内存，严重降低性能。现有算法未能有效解决这一瓶颈。

Method: 提出gMAGNUS算法，核心是执行中间乘积的**行内重排序**，将重型行细分为多个独立块，使得每个块可以完全在局部内存中完成累加。该重排序基于新颖的**外积操作**和**分层多分割（hierarchical multisplit）**实现。算法具备输入感知和系统感知能力，能够根据输入矩阵维度和局部内存大小自动确定块数量和分层多分割层数。

Result: 在两个大规模数据集上，gMAGNUS相比MKL、cuSPARSE等五种领先算法获得1.81至7.62倍的几何平均加速比；其核心内核性能接近理论上限。

Conclusion: gMAGNUS通过智能的行内重排序和分层多分割策略，有效解决了不规则稀疏矩阵SpGEMM中重型行导致的局部内存溢出问题，实现了显著的性能提升，并具备良好的输入与系统自适应性。
...
### [451] [Application-Driven Architecture Exploration for Cross-Layer Heterogeneous Systems](https://arxiv.org/abs/2607.23042)
...
TL;DR: CHASE是一个面向跨层异构系统(XHS)设计的应用驱动框架，通过解耦的两级搜索(内层映射+外层优化)在满足物理部署约束的前提下自动搜索最优硬件架构，在稀疏计算和LLM工作负载上分别实现6.20×和2.12×的几何平均加速比。


<details>
  <summary>Details</summary>
Motivation: AI和HPC基础设施需要同时支持密集张量计算、稀疏核、大内存和通信密集型集合操作等多种工作负载组合，而跨层异构系统(XHS)的设计空间涉及加速器、内存层级、扩展网络和集群网络等多层面的协同选择，且硬件选择会改变合法任务映射，加上机架功耗、交换机辐度、布线和成本等约束使大多候选方案无效，因此亟需一种能够自动搜索可行且高效XHS架构的方法。

Method: 采用解耦的两级搜索循环：内层映射器将硬件无关的工作负载DAG转换为拓扑感知的事件迹，并通过校准的事件驱动模拟器评估；外层遥测引导的优化器进化硬件图。候选架构表示为层次化类型图，拒绝违反部署约束的设计。内层映射器采用高效搜索策略，避免全局联合硬件-映射搜索的复杂性问题。

Result: 映射器在接近穷举最优值6.06%范围内，平均映射时间减少60.5%；计算模型误差为4.4-7.5%；通信验证在物理平台上重现关键趋势；外层搜索在64次迭代内达到接近全局最优。端到端案例表明：稀疏工作负载偏好关键性感知的异构pod，LLM推理偏好扩展岛设计，分别实现6.20×和2.12×几何平均加速比，同时降低成本和功耗。

Conclusion: CHASE框架能够有效探索复杂的跨层异构系统设计空间，通过解耦搜索策略在可行时间内找到接近最优的架构设计，且针对不同类型工作负载能自动发现差异化的最优架构模式（稀疏任务偏异构pod、LLM偏扩展岛），验证了应用驱动设计方法的有效性。
...
### [452] [Gleam: Adaptive Network-Efficient CUDA API Remoting for Cross-Device GPU Sharing over LANs](https://arxiv.org/abs/2607.23115)
...
TL;DR: 本文提出Gleam，一个面向局域网内异构个人设备的网络高效GPU共享框架。通过CUDA API远程调用实现分布式任务卸载，并针对带宽瓶颈、高频API调用和跨任务竞争问题，采用模型权重缓存、异步执行、运行时任务调度器及CUDA上下文一致性机制，在异构GPU和多样化AI负载下实现1.4-24.2倍的API远程调用效率提升和最高1.79倍的系统吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 现有局域网内GPU共享方案受限于网络带宽瓶颈、高频API调用带来的累积延迟以及并行任务间的资源竞争，导致计算通信效率低下，难以在异构个人设备上实现高效的分布式AI推理。

Method: (1) 自动模型权重缓存减少带宽开销，异步执行缓解频繁API调用的累积延迟；(2) 运行时任务调度器动态确定API远程调用配对，同时考虑网络条件和GPU资源竞争；(3) 专用机制保证分布式执行中的CUDA上下文一致性。

Result: 在异构NVIDIA GPU和多种AI工作负载上，Gleam对比最先进基线方案，API远程调用效率提升1.4-24.2倍，系统吞吐量最高提升1.79倍。

Conclusion: 通过解决网络瓶颈问题，Gleam实现了局域网内高效、通用的GPU共享，为异构个人设备上的无处不在的AI推理提供了可行方案。
...
### [453] [Libra: Taming Attention Workload Skew in Long-Context LLM Training with Bounded Sequence Pool](https://arxiv.org/abs/2607.23250)
...
TL;DR: Libra提出了一种基于大数定律的负载均衡方法，通过固定大小的序列池和方差缩减的序列放置策略，解决了长上下文LLM训练中因注意力计算负载不均导致的 straggler 和 pipeline 气泡问题，在Qwen3-Turbo训练中实现了最高2.54倍的端到端吞吐提升。


<details>
  <summary>Details</summary>
Motivation: 长上下文LLM训练存在负载均衡问题——将样本打包成固定token序列虽能平衡内存和线性算子成本，但主导计算开销的注意力机制其复杂度与序列长度平方和成正比。长尾分布语料中，等长打包序列的注意力负载差异巨大，导致数据并行中的 straggler 和 pipeline 气泡。现有方法要么在序列或微批次粒度上均衡（异常值会主导分配），要么将注意力分解到全局工作池（通信域随DP规模增长）。

Method: Libra将大数定律作为负载均衡的缩放原则：注意力均衡池的大小无需随DP度增长。具体包括：(1) 将打包序列及其上下文并行组分组为固定大小的序列池，DP扩展时增加池数量而非扩大单个池；(2) 方差缩减的序列放置（Variance-Reduced Sequence Placement），将注意力负载互补的序列放在同一池中以减小池间残差倾斜；(3) 平铺注意力池化（Tiled Attention Pooling），在池内将序列-头SH-Tiles分配到各GPU，配合流水线运行时重叠tile交换与注意力计算。

Result: 在Qwen3-Turbo训练的256K和1M token负载上，相比Ulysses，Libra将端到端吞吐提升最高2.54倍，微基准测试中最差步的 straggler 注意力加速比达3.14倍。已在生产环境中运行数十万GPU小时，支持32K至1M token的作业，且保持训练语义不变。

Conclusion: Libra通过精巧的池化设计和方差缩减放置策略，在不改变模型层、优化器或pipeline调度的情况下，有效解决了长上下文训练中的注意力负载均衡问题，实现了显著的端到端性能提升，并在大规模生产环境中得到验证。
...
### [454] [X-Stage: An Overlooked Pipeline Stage for Communication-Computation Overlap in DiT Inference](https://arxiv.org/abs/2607.23264)
...
TL;DR: 本文发现并利用分布式DiT推理中GPU内核通信的"X-Stage"（软件可见的发布后流水线阶段），提出Burst-Gap模型来预测通信开销和背压；基于模型重新设计了DeepGEMM MegaMoE和Ulysses序列并行注意力两个融合内核，最大加速比达1.62x和1.43x。


<details>
  <summary>Details</summary>
Motivation: 现有系统调度通信的发起和接收数据的可用时机，但忽略了发布后进度（post-issue progress）在远程可见完成前的行为，导致发送端背压难以预测，影响通信计算重叠效率。

Method: 1) 识别X-Stage（软件可见的发布后流水线阶段）；2) 在8-GPU NVIDIA架构上测量发现短突发可被计算掩盖，持续注入会耗尽容量导致延迟；3) 提出Burst-Gap模型（参数：无背压发起时间、有效排出速率、未完成容量）；4) 基于模型重新设计DeepGEMM MegaMoE（跨专家波交错Linear-1/Linear-2）和Ulysses注意力（tile粒度融合All-to-All与FlashAttention）。

Result: DeepGEMM MegaMoE：84个配置上几何平均加速1.18x，最大1.62x；Ulysses注意力：FlashAttention-3达1.43x，FlashAttention-4达1.42x发送端可见加速，长序列稳态时间接近纯FlashAttention。

Conclusion: 后发布进度（X-Stage）是可测量的调度杠杆，可用于塑造突发、避免背压、隐藏发送端开销。
...
### [455] [SpecBox: Speculative Sandbox Scheduling for Efficient LLM Agent Serving](https://arxiv.org/abs/2607.23933)
*Yihui Zhang,Tianyu Wo,Jinghao Wang,Xiaoyang Sun,Menghao Zhang,Cangzhou Yuan,Li Li,Chunming Hu,Albert Y. Zomaya,Renyu Yang*

Main category: cs.DC

TL;DR: SpecBox 是一个面向 LLM 智能体执行管道的运行时系统，通过推测性沙箱预分配（关键词匹配+流式语义嵌入实现意图驱动预预热、上下文感知随机预取）、语义结果缓存和带外共享内存传输平面，在冷启动延迟和内存开销之间取得平衡，相比按需沙箱将 P99 延迟降低 2.9 倍，相比永久预留沙箱将峰值内存降低 45.9%。


<details>
  <summary>Details</summary>
Motivation: LLM 智能体依赖 MCP 调用隔离外部沙箱时，分解式沙箱部署存在资源利用率与交互尾部延迟之间的根本矛盾：持久化的长生命周期沙箱预留在大规模下产生过高内存开销，而按需即时实例化则引入严重冷启动惩罚，损害多租户多轮智能体工作负载下的响应性能。

Method: 1) 推测性沙箱预分配：基于关键词匹配和流式语义嵌入的意图驱动沙箱预预热，在 LLM 生成过程中识别待执行的工具调用需求，使沙箱启动与模型推理完全重叠；2) 上下文感知随机预取：在沙箱依赖图上进行概率性预取，将预预热窗口延伸至连续智能体步骤；3) 语义结果缓存：剪枝冗余的重复沙箱调用；4) 带外共享内存传输平面：绕过传统网络序列化，实现零拷贝工件传输。

Result: 在高并发多轮智能体轨迹上，相比按需沙箱基线，P99 端到端延迟降低 2.9 倍；相比永久预留沙箱部署，峰值内存消耗降低 45.9%。

Conclusion: SpecBox 通过推测性预分配机制有效化解了 LLM 智能体沙箱部署中资源利用率与延迟之间的 trade-off，验证了意图驱动预预热与上下文感知预取在实际多租户多轮工作负载中的有效性。
...
### [456] [Consensus In Asynchrony: Strictly Formal](https://arxiv.org/abs/2607.24095)
*Ivan Klianev*

Main category: cs.DC

TL;DR: 本文解决了异步环境下确定性崩溃容错共识的可能性与FLP不可能性结果之间的表面矛盾，通过严格的形式化框架证明仅需一个协议阶段即可跨越这一理论鸿沟，并提出了一个能容忍多个崩溃故障的新算法。


<details>
  <summary>Details</summary>
Motivation: 解决异步系统中确定性崩溃容错共识的可能性与FLP不可能性结果（由Attiya, Castañeda和Rajsbaum重新确认）之间的明显矛盾，弥合这一基础性的理论鸿沟。

Method: 采用严格的形式化框架扩展前人的推理，证明仅需一个协议阶段即可从FLP不可能性结论转向相反的确定性共识可能性结论。

Result: 提出了一个能够容忍多个崩溃故障的新型算法，并提供了严格的形式化正确性证明。

Conclusion: 通过形式化框架澄清了异步环境下崩溃容错共识的理论基础，证明了确定性共识在理论上是可以实现的。
...
### [457] [Certified Parallel-in-Time Sinkhorn for Dynamic Entropic Optimal Transport](https://arxiv.org/abs/2607.24741)
*Xinyang Wen*

Main category: cs.DC

TL;DR: TemporalSinkhorn是一种并行时间执行器，通过批处理未来候选解及其修正项、使用确定性安全前缀与在线投影遗忘率审计里程碑，加速动态最优传输中的Sinkhorn迭代，在多个GPU和Flow Matching场景下实现1.15x-4.315x的加速比且无精度违规。


<details>
  <summary>Details</summary>
Motivation: 动态应用（如最优传输Flow Matching）需要反复求解相关的熵最优传输问题，但传统的分布式Sinkhorn过程按顺序处理帧并在每次迭代后同步，导致计算效率低下。

Method: 提出TemporalSinkhorn，一种并行时间执行器。核心包括：(1) 批处理未来候选解及其修正项；(2) 使用居中的行分片证书仅接受确定性安全前缀；(3) 打包的Sinkhorn更新在剩余候选中共享；(4) 在线投影遗忘率设置审计里程碑；(5) 后验残差检查用于恢复深度低估。预测可改变工作放置但不能授权不准确输出。

Result: 在4块A100 GPU上，n=2048的60轮五种子网格实验中，遗忘引导的里程碑相比每次打包迭代审计减少了1.15x-1.47x的墙钟时间；相比顺序软c-变换热启动，时间执行在六个合成数据流上快1.42x-3.55x且零边际容忍违规；在Flow Matching小批量流上快3.054x-3.632x；在RTX 4060笔记本GPU上固定核测试几何平均加速比达4.315x。

Conclusion: TemporalSinkhorn通过并行时间执行显著加速了动态Sinkhorn过程，无精度损失。但端到端Flow Matching集成、优化求解器比较和多节点验证仍为待完成的工作。
